...
App* g=nullptr;
struct UiItem { HWND hwnd{}; int x{},y{},w{},h{}; };
UINT gDpi=96;
double gUiScale=1.0;
std::vector<UiItem> gUi;
HFONT gUiFont=nullptr;
int dpiPx(int v){return int(std::lround(double(v)*gUiScale));}
void updateUiScaleForWorkArea(HWND hwnd,UINT dpi){
    RECT work{};bool ok=false;
    if(hwnd){MONITORINFO mi{sizeof(mi)};auto mon=MonitorFromWindow(hwnd,MONITOR_DEFAULTTONEAREST);if(mon&&GetMonitorInfoW(mon,&mi)){work=mi.rcWork;ok=true;}}
    if(!ok)ok=SystemParametersInfoW(SPI_GETWORKAREA,0,&work,0)!=FALSE;
    const double dpiScale=std::max(0.80,double(dpi)/96.0);
    if(!ok){gUiScale=dpiScale;return;}
    const double aw=std::max(640L,work.right-work.left-24L), ah=std::max(480L,work.bottom-work.top-24L);
    const double fit=std::min(aw/1000.0,ah/700.0);
    gUiScale=std::max(0.80,std::min(dpiScale,fit));
}
void rebuildUiFont(){
    if(gUiFont){DeleteObject(gUiFont);gUiFont=nullptr;}
    gUiFont=CreateFontW(-std::max(11,int(std::lround(10.0*96.0*gUiScale/72.0))),0,0,0,FW_NORMAL,FALSE,FALSE,FALSE,DEFAULT_CHARSET,OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,CLEARTYPE_QUALITY,DEFAULT_PITCH|FF_DONTCARE,L"Segoe UI");
    for(const auto&i:gUi)if(i.hwnd)SendMessageW(i.hwnd,WM_SETFONT,(WPARAM)gUiFont,TRUE);
}
void reflowUi(){for(const auto&i:gUi)if(i.hwnd)SetWindowPos(i.hwnd,nullptr,dpiPx(i.x),dpiPx(i.y),dpiPx(i.w),dpiPx(i.h),SWP_NOZORDER|SWP_NOACTIVATE);}
void setText(HWND h,const std::wstring&s){SetWindowTextW(h,s.c_str());}
HWND add(HWND p,const wchar_t* cls,const wchar_t* text,DWORD style,int x,int y,int w,int h,int id){
    HWND c=CreateWindowExW(0,cls,text,WS_CHILD|WS_VISIBLE|style,dpiPx(x),dpiPx(y),dpiPx(w),dpiPx(h),p,(HMENU)(INT_PTR)id,GetModuleHandleW(nullptr),nullptr);
    if(c){gUi.push_back({c,x,y,w,h});if(gUiFont)SendMessageW(c,WM_SETFONT,(WPARAM)gUiFont,TRUE);}
    return c;
}
...
void renderLoop(){double previousMs=0.0;bool deadlineMiss=false;while(g&&g->running.load()){
    ...
    RenderAudio a;auto p=g->policy;p.multiOut=true;p.lookahead=std::min(.08f,float(dec.quantumMs)/1000.f);auto t0=std::chrono::steady_clock::now();bool ok=g->renderer.render(ev,start,end,g->sr,p,g->req.fetch_add(1),a);previousMs=std::chrono::duration<double,std::milli>(std::chrono::steady_clock::now()-t0).count();g->lastRenderMs.store(previousMs);deadlineMiss=previousMs>dec.quantumMs;
...
void buildUi(HWND w){
    ...
    add(w,L"STATIC",L"Retake",0,650,100,60,20,-1);HWND rt=add(w,WC_COMBOBOXW,L"",CBS_DROPDOWNLIST,710,96,130,150,IDC_RETAKE);for(auto*s:{L"Off",L"Timbre",L"Dynamics",L"Vibrato",L"Micro-Pitch",L"Timing Feel",L"Bow / Attack",L"All"})comboAdd(rt,s);SendMessageW(rt,CB_SETCURSEL,0,0);add(w,L"BUTTON",L"New Take",BS_PUSHBUTTON,850,95,106,25,IDC_NEW_TAKE);
    add(w,L"BUTTON",L"Smart Dynamics",BS_AUTOCHECKBOX,18,136,140,24,IDC_SMART_DYN);add(w,L"BUTTON",L"Smart Articulation",BS_AUTOCHECKBOX,166,136,150,24,IDC_SMART_ART);HWND poly=add(w,L"BUTTON",L"Independent Polyphony",BS_AUTOCHECKBOX,324,136,170,24,IDC_POLY);SendMessageW(poly,BM_SETCHECK,BST_CHECKED,0);HWND auth=add(w,L"BUTTON",L"MIDI Authority Lock",BS_AUTOCHECKBOX,500,136,150,24,IDC_AUTH_LOCK);SendMessageW(auth,BM_SETCHECK,BST_CHECKED,0);HWND phr=add(w,L"BUTTON",L"Phrase Director",BS_AUTOCHECKBOX,656,136,130,24,IDC_PHRASE);SendMessageW(phr,BM_SETCHECK,BST_CHECKED,0);
    add(w,L"STATIC",L"Retake Amount",0,18,174,100,20,-1);HWND ra=add(w,TRACKBAR_CLASSW,L"",TBS_AUTOTICKS,118,168,250,30,IDC_RETAKE_AMOUNT);SendMessageW(ra,TBM_SETRANGE,TRUE,MAKELPARAM(0,100));SendMessageW(ra,TBM_SETPOS,TRUE,0);add(w,L"STATIC",L"Ensemble Looseness",0,390,174,130,20,-1);HWND loose=add(w,TRACKBAR_CLASSW,L"",TBS_AUTOTICKS,520,168,250,30,IDC_LOOSE);SendMessageW(loose,TBM_SETRANGE,TRUE,MAKELPARAM(0,100));SendMessageW(loose,TBM_SETPOS,TRUE,18);
    add(w,L"STATIC",L"SCORING STAGE MIXER · Master + 16 independent virtual-geometry aux feeds.",0,18,210,900,24,-1);
    const wchar_t* names[]={L"Master",L"Spot L",L"Spot C",L"Spot R",L"Tree L",L"Tree C",L"Tree R",L"Wide L",L"Wide R",L"Room L",L"Room R",L"Rear",L"Mid L",L"Mid R",L"Far L",L"Far R",L"Gallery"};
    for(int i=0;i<17;++i){int col=i/6,row=i%6,x=18+col*320,y=244+row*44;add(w,L"STATIC",names[i],0,x,y,72,20,-1);HWND sl=add(w,TRACKBAR_CLASSW,L"",TBS_AUTOTICKS,x+74,y-4,225,32,i==0?IDC_MASTER:IDC_FEED0+i-1);SendMessageW(sl,TBM_SETRANGE,TRUE,MAKELPARAM(0,100));SendMessageW(sl,TBM_SETPOS,TRUE,i==0?100:0);}
...
case WM_DESTROY:KillTimer(w,1);closeMidi();stopEngine();if(gUiFont){DeleteObject(gUiFont);gUiFont=nullptr;}gUi.clear();PostQuitMessage(0);return 0;}return DefWindowProcW(w,msg,wp,lp);}
}

int WINAPI wWinMain(HINSTANCE h,HINSTANCE,LPWSTR,int show){
    SetProcessDpiAwarenessContext(DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE_V2);
    gDpi=GetDpiForSystem();updateUiScaleForWorkArea(nullptr,gDpi);
    INITCOMMONCONTROLSEX ic{sizeof(ic),ICC_BAR_CLASSES};InitCommonControlsEx(&ic);App app;g=&app;WNDCLASSW wc{};wc.lpfnWndProc=proc;wc.hInstance=h;wc.lpszClassName=L"SonicraftProductShellV70";wc.hCursor=LoadCursor(nullptr,IDC_ARROW);wc.hbrBackground=(HBRUSH)(COLOR_WINDOW+1);RegisterClassW(&wc);
    HWND w=CreateWindowExW(0,wc.lpszClassName,L"SONICRAFT AI Strings Q4 · v7.0 RC2 · Frontend Lock",WS_OVERLAPPED|WS_CAPTION|WS_SYSMENU|WS_MINIMIZEBOX,CW_USEDEFAULT,CW_USEDEFAULT,dpiPx(1000),dpiPx(700),nullptr,nullptr,h,nullptr);if(!w)return 2;ShowWindow(w,show);UpdateWindow(w);MSG m{};while(GetMessageW(&m,nullptr,0,0)>0){TranslateMessage(&m);DispatchMessageW(&m);}g=nullptr;return int(m.wParam);}
#endif
