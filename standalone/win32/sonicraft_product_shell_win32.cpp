...
struct App {
    ...
    uint32_t sr=48000; int quantumMs=160; int64_t quantumFrames=7680; std::atomic<uint64_t> req{240000};
...
void CALLBACK midiCb(HMIDIIN,UINT msg,DWORD_PTR instance,DWORD_PTR p1,DWORD_PTR){if(msg==MIM_DATA){auto*a=(App*)instance;if(a&&a->hwnd)PostMessageW(a->hwnd,WM_APP_MIDI,WPARAM(p1),0);}}
void openMidi(){closeMidi();if(!g)return;HWND cb=GetDlgItem(g->hwnd,IDC_MIDI);int sel=comboSel(cb);if(sel<0)return;g->midiDevice=UINT(sel);if(midiInOpen(&g->midi,g->midiDevice,(DWORD_PTR)midiCb,(DWORD_PTR)g,CALLBACK_FUNCTION)==MMSYSERR_NOERROR)midiInStart(g->midi);else g->midi=nullptr;}
...
size_t queuedAudio(){if(!g)return 0;cleanupAudio(false);std::lock_guard<std::mutex>lk(g->audioMu);return g->audio.size();}
...
void renderLoop(){while(g&&g->running.load()){size_t q=queuedAudio();bool active=g->timeline.anyActiveNotes();if(active)g->tailBlocks=3;if(!active&&g->tailBlocks<=0){std::this_thread::sleep_for(std::chrono::milliseconds(10));continue;}if(q>=3){std::this_thread::sleep_for(std::chrono::milliseconds(3));continue;}if(q==0&&g->renderCursor.load()>0)++g->underruns;int64_t start=g->renderCursor.load(),end=start+g->quantumFrames;auto ev=g->timeline.contextFor(start,end,int64_t(g->sr)*5);if(ev.empty()){std::this_thread::sleep_for(std::chrono::milliseconds(4));continue;}RenderAudio a;auto p=g->policy;p.multiOut=true;auto t0=std::chrono::steady_clock::now();bool ok=g->client.render(ev,start,end,g->sr,p,g->req.fetch_add(1),a);if(ok){auto y=mixToStereo(a,g->mixer);queueWave(y);g->renderCursor.store(end);if(!active)--g->tailBlocks;}else std::this_thread::sleep_for(std::chrono::milliseconds(25));auto ms=std::chrono::duration<double,std::milli>(std::chrono::steady_clock::now()-t0).count();if(ms>g->quantumMs*1.25)PostMessageW(g->hwnd,WM_TIMER,2,0);}cleanupAudio(false);}}
void startEngine(){if(!g||g->running.load())return;g->timeline.reset();g->renderCursor=0;g->tailBlocks=0;g->underruns=0;if(!openWave()){MessageBoxW(g->hwnd,L"Could not open the selected audio output.",L"SONICRAFT",MB_ICONERROR);return;}g->running=true;g->worker=std::thread(renderLoop);}
void stopEngine(){if(!g)return;g->running=false;if(g->worker.joinable())g->worker.join();closeWave();g->timeline.reset();}

void populateDevices(HWND w){HWND m=GetDlgItem(w,IDC_MIDI);SendMessageW(m,CB_RESETCONTENT,0,0);UINT nm=midiInGetNumDevs();for(UINT i=0;i<nm;++i){MIDIINCAPSW c{};if(midiInGetDevCapsW(i,&c,sizeof(c))==MMSYSERR_NOERROR)comboAdd(m,c.szPname);}if(nm)SendMessageW(m,CB_SETCURSEL,0,0);HWND a=GetDlgItem(w,IDC_AUDIO);SendMessageW(a,CB_RESETCONTENT,0,0);comboAdd(a,L"Windows Default");UINT na=waveOutGetNumDevs();for(UINT i=0;i<na;++i){WAVEOUTCAPSW c{};if(waveOutGetDevCapsW(i,&c,sizeof(c))==MMSYSERR_NOERROR)comboAdd(a,c.szPname);}SendMessageW(a,CB_SETCURSEL,0,0);}
...
void buildUi(HWND w){
    add(w,L"STATIC",L"SONICRAFT AI STRINGS · v2.4 REALTIME PRODUCT SHELL",SS_LEFT,18,14,640,28,-1);
    ...
    add(w,L"STATIC",L"Realtime Preview uses rolling 160 ms neural windows. Final quality still uses the normal AUTO/HQ acoustic-promotion path.",0,18,550,920,26,-1);
...
case WM_APP_MIDI:{DWORD m=DWORD(wp);uint8_t st=uint8_t(m&255),d1=uint8_t((m>>8)&255),d2=uint8_t((m>>16)&255);int64_t s=g->renderCursor.load()+g->quantumFrames/2;g->timeline.pushMidiShort(st,d1,d2,s,g->policy.tempo);return 0;}
case WM_COMMAND:{int id=LOWORD(wp);if(id==IDC_MIDI&&HIWORD(wp)==CBN_SELCHANGE){if(g->running)openMidi();}else if(id==IDC_AUDIO&&HIWORD(wp)==CBN_SELCHANGE){int s=comboSel(GetDlgItem(w,IDC_AUDIO));g->audioDevice=s<=0?WAVE_MAPPER:UINT(s-1);if(g->running){stopEngine();startEngine();openMidi();}}else if(id==IDC_START){if(g->running){closeMidi();stopEngine();setText(GetDlgItem(w,IDC_START),L"Start Realtime Preview");}else{syncPolicy(w);startEngine();openMidi();setText(GetDlgItem(w,IDC_START),L"Stop Realtime Preview");}}else if(id==IDC_NEW_TAKE){g->policy.retakeNonce=(g->policy.retakeNonce+1)&255;}else syncPolicy(w);return 0;}
case WM_HSCROLL:syncPolicy(w);return 0;
case WM_TIMER:{syncPolicy(w);bool ready=g->client.ping(g->sr);std::wstring t=ready?L"Service: READY":L"Service: OFFLINE";t+=L"  · queued="+std::to_wstring(queuedAudio())+L" · underruns="+std::to_wstring(g->underruns)+L" · retake seed="+std::to_wstring(g->policy.retakeNonce);setText(GetDlgItem(w,IDC_STATUS),t);return 0;}
case WM_DESTROY:KillTimer(w,1);closeMidi();stopEngine();PostQuitMessage(0);return 0;}return DefWindowProcW(w,msg,wp,lp);}
}

int WINAPI wWinMain(HINSTANCE h,HINSTANCE,LPWSTR,int show){INITCOMMONCONTROLSEX ic{sizeof(ic),ICC_BAR_CLASSES};InitCommonControlsEx(&ic);App app;g=&app;WNDCLASSW wc{};wc.lpfnWndProc=proc;wc.hInstance=h;wc.lpszClassName=L"SonicraftProductShellV24";wc.hCursor=LoadCursor(nullptr,IDC_ARROW);wc.hbrBackground=(HBRUSH)(COLOR_WINDOW+1);RegisterClassW(&wc);HWND w=CreateWindowExW(0,wc.lpszClassName,L"SONICRAFT AI Strings Q4 · Realtime Product Shell",WS_OVERLAPPED|WS_CAPTION|WS_SYSMENU|WS_MINIMIZEBOX,CW_USEDEFAULT,CW_USEDEFAULT,990,680,nullptr,nullptr,h,nullptr);if(!w)return 2;ShowWindow(w,show);UpdateWindow(w);MSG m{};while(GetMessageW(&m,nullptr,0,0)>0){TranslateMessage(&m);DispatchMessageW(&m);}g=nullptr;return int(m.wParam);}
#endif
