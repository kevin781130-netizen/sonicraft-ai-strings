...
def main():
    ...
    ap.add_argument('--seconds',type=float,default=2.0);ap.add_argument('--source-fingerprint',default=None);a=ap.parse_args()
    ...
    for i,r in enumerate(tqdm(rows)):
        ...
        outrows.append({'file':str(fn.resolve()),'dataset':r['dataset'],'release_blocked':bool(r.get('release_blocked',False)),
                        'commercial_safe':r.get('commercial_safe'),
                        'cleanroom_eligible':r.get('cleanroom_eligible'),
                        ...
                        'data_fingerprint':a.source_fingerprint,
    ...
    def sha256_file(p):
        ...
        return h.hexdigest()
    provenance={
        'schema':'sonicraft-vae64-latent-provenance-v1',
        'source_index':str(Path(a.index).resolve()),
        'source_index_sha256':sha256_file(a.index),
        'codec':str(Path(a.codec).resolve()),
        'codec_sha256':sha256_file(a.codec),
        'source_fingerprint':a.source_fingerprint,
        'rows':len(outrows),
        'latent_ch':int(model.latent_dim),
        'latent_hz':float(model.latent_hz),
    ...
    print('VAE64 latent segments',len(outrows),idx,'geometry',model.latent_dim,'@',model.latent_hz,'Hz','fingerprint',a.source_fingerprint)
