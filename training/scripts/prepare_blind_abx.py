...
def main():
    ...
    rng=random.Random(a.seed); legacy=[]; answers=[]; rows=[]
    for i,stem in enumerate(stems,1):
        ...
        a_sha=sha256(pub/namea); b_sha=sha256(pub/nameb)
        rows.append({'listener_id':'','trial_id':tid,'A_file':namea,'B_file':nameb,'pick_generated':'','confidence_1_5':'','notes':''})
        legacy.append({'trial_id':tid,'source_stem':stem,'generated_side':generated_side,'A_sha256':a_sha,'B_sha256':b_sha})
        answers.append({'trial_id':tid,'trial_kind':'target','answer':generated_side,'source_stem':stem,'A_file':namea,'B_file':nameb,
                        'A_sha256':a_sha,'B_sha256':b_sha})
    with (pub/'responses.csv').open('w',newline='',encoding='utf-8-sig') as f:
        w=csv.DictWriter(f,fieldnames=rows[0].keys()); w.writeheader(); w.writerows(rows)
    (pub/'README.txt').write_text(
        'Blind real-vs-generated test. For every trial, listen to A and B and enter A or B under pick_generated. '
        'Enter one stable listener_id on every row. Give each listener a separate copy of responses.csv; finished CSV files may later be placed in one directory and scored together. '
        'Do not inspect the private folder until all responses are locked. Use only held-out real recordings and generated renders never used for training.\n',
        encoding='utf-8')
    (prv/'answer_key.json').write_text(json.dumps({'schema':2,'seed':a.seed,'answers':answers,'trials':legacy},indent=2),encoding='utf-8')
