...
STAGES=("codec","renderer","distill","shortcut")
...
def stage_identity(c, stage):
    # Target epoch counts are intentionally excluded so a stage can be extended later
    # without invalidating an otherwise compatible checkpoint.
    if stage=="codec":
        return {"stage":"codec","width":int(c["codec"]["width"]),"batch":int(c["codec"]["batch"])}
    if stage=="renderer":
        return {"stage":"renderer","preset":str(c["renderer"]["preset"]),
                "batch":int(c["renderer"]["batch"]),"accum":int(c["renderer"]["accum"])}
    if stage=="distill":
        return {"stage":"distill","preset":str(c["distill"]["preset"]),
                "batch":int(c["distill"]["batch"]),"accum":int(c["distill"]["accum"])}
    if stage=="shortcut":
        return {"stage":"shortcut","preset":str(c["shortcut"]["preset"]),
                "batch":int(c["shortcut"]["batch"]),"accum":int(c["shortcut"]["accum"]),
                "max_steps":int(c["shortcut"]["max_steps"]),
                "recommend_steps":int(c["shortcut"]["recommend_steps"])}
...
def stage_fingerprint(c, stage):
    return hash_obj(stage_identity(c,stage))

def stage_fingerprints(c):
    return {s:stage_fingerprint(c,s) for s in STAGES}

def fingerprint(c):
    # Overall display/audit fingerprint; checkpoints use per-stage hashes.
    return hash_obj({"schema":c.get("schema"),"stages":stage_fingerprints(c)})

def q(v):
    return str(v).replace('"','')

def cmd_lines(c):
    sf=stage_fingerprints(c)
    vals={
        ...
        "SONICRAFT_RECIPE_FINGERPRINT":fingerprint(c),
        "DNNI_CODEC_RECIPE_FINGERPRINT":sf["codec"],
        "DNNI_RENDER_RECIPE_FINGERPRINT":sf["renderer"],
        "DNNI_DISTILL_RECIPE_FINGERPRINT":sf["distill"],
        "DNNI_SHORTCUT_RECIPE_FINGERPRINT":sf["shortcut"],
    }
...
def main():
    ...
    ap.add_argument("--write-cmd")
    ap.add_argument("--fingerprint",action="store_true")
    ap.add_argument("--stage-fingerprint",choices=STAGES)
    a=ap.parse_args();c=load(a.config)
    if a.stage_fingerprint:
        print(stage_fingerprint(c,a.stage_fingerprint));return
    if a.fingerprint:
        print(fingerprint(c));return
    lines=cmd_lines(c)
    if a.write_cmd:
        p=Path(a.write_cmd);p.parent.mkdir(parents=True,exist_ok=True)
        p.write_text("\r\n".join(lines)+"\r\n",encoding="utf-8")
