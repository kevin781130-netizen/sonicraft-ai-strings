...
VERSION8='2.1.0-phrase-transition-promotion'
POLICY5={
...
def normalized_training_policy(pj,schema:int):
    p=dict(pj.get('training_policy') or {})
    required=POLICY8 if schema>=8 else (POLICY7 if schema>=7 else (POLICY6 if schema>=6 else POLICY5))
...
def stage_evidence(md:Path,path:Path,key:str,extra:dict|None=None,dest_name:str|None=None):
    dst=md/(dest_name or path.name)
...
def main():
    ...
    ap.add_argument('--schema',type=int,choices=(5,6,7,8),default=6)
    ...
    expected_curriculum=POLICY8['curriculum'] if schema>=8 else (POLICY7['curriculum'] if schema>=7 else (POLICY6['curriculum'] if schema>=6 else POLICY5['curriculum']))
    choices=[
    ...
    if schema>=8:
        ...
        cp=Path(a.phrase_curriculum_report); hp=Path(a.hq_transition_promotion); kp=Path(a.compact_transition_promotion)
        ...
        for f in out:
            role=f['role']
            if role not in reports: continue
            rp,rr=reports[role]; pid=str(rr.get('promotion_id','')).lower(); ck=torch.load(md/f['name'],map_location='cpu',weights_only=False)
            if not isinstance(ck,dict): raise SystemExit(f'{role} checkpoint metadata invalid for schema 8: {f["name"]}')
            if str(ck.get('transition_promotion_id','')).lower()!=pid: raise SystemExit(f'{role} checkpoint transition_promotion_id mismatch: {f["name"]}')
            seal=dict(ck.get('transition_promotion_seal') or {})
            rsha=sha(rp); td=tensor_digest(ck)
            if int(seal.get('schema',0))!=1 or str(seal.get('promotion_id','')).lower()!=pid: raise SystemExit(f'{role} transition seal identity mismatch: {f["name"]}')
            if str(seal.get('promotion_sha256','')).lower()!=rsha: raise SystemExit(f'{role} transition seal promotion SHA mismatch: {f["name"]}')
            if str(seal.get('candidate_checkpoint_sha256','')).lower()!=str(rr.get('candidate_checkpoint_sha256','')).lower(): raise SystemExit(f'{role} transition seal candidate SHA mismatch: {f["name"]}')
            if str(seal.get('curriculum_sha256','')).lower()!=csha: raise SystemExit(f'{role} transition seal curriculum SHA mismatch: {f["name"]}')
            if str(seal.get('heldout_index_sha256','')).lower()!=str(rr.get('heldout_index_sha256','')).lower(): raise SystemExit(f'{role} transition seal held-out index mismatch: {f["name"]}')
            if str(seal.get('tensor_sha256','')).lower()!=td: raise SystemExit(f'{role} transition seal tensor digest mismatch: {f["name"]}')
    ...
    version=VERSION8 if schema>=8 else (VERSION7 if schema>=7 else (VERSION6 if schema>=6 else VERSION5))
