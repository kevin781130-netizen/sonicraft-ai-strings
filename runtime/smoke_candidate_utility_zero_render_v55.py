...
with tempfile.TemporaryDirectory() as td:
    ...
    assert row["candidate_renders_skipped"]>=2,row
    assert row["candidate_renders_escalated"]==0,row
    assert Path(row["candidate_utility_sidecar"]).exists()
    d0=row["decisions"][0]
    assert d0["candidate_budget"]["utility_reason"]=="high_conf_top1_plus_D",d0
    assert d0["candidate_budget"]["primary_slots"]==["B","D"],d0
    assert set(d0["candidate_budget"]["deferred_slots"])=={"A","C"},d0
    assert d0["candidate_budget"]["expanded"] is False
    assert set(d0["local_wavs"])=={"B","D"},d0
    # Memory only learned actually rendered B/D in this accepted high-confidence pass.
    m2=CandidateUtilityMemoryV55(utility);ctx=m2.context(key)["slots"]
    assert ctx["B"]["evidence"]>10 and ctx["D"]["evidence"]>10
    assert abs(ctx["A"]["evidence"]-10)<1e-9 and abs(ctx["C"]["evidence"]-10)<1e-9
    ...
    print("SONICRAFT v5.5 high-confidence ZERO-RENDER pruning smoke OK",
