...
import numpy as np
import auto_loop_strings_v61 as loop
from compile_musicxml_strings_v61 import compile_file as real_compile
...
from performance_checkpoint_v61 import load_checkpoint_v61,replay_verify_checkpoint_v61,verify_checkpoint_v61
...
with tempfile.TemporaryDirectory() as td:
    ...
    try:
        report,rp=loop.run_auto_loop_v61(
    ...
    # v6.1 checkpoint is auto-created from the PRE-round evidence/policy state and finalized after the round.
    cp_path=Path(report["final"]["performance_checkpoint"])
    assert cp_path.exists(),report["final"]
    cp=load_checkpoint_v61(cp_path)
    ...
    cv=verify_checkpoint_v61(cp_path,score,estore,current_policy_path=td/"policy.json",artifact_dir=td/"out")
    ...
    # Replay itself is covered by smoke_performance_checkpoint_v61.py using the unmodified real compiler.
    ...
    print("SONICRAFT v6.1 Auto-Loop checkpoint + replay + evidence recovery smoke OK",
