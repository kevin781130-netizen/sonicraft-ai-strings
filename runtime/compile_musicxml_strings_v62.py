"""SONICRAFT v6.2 Reproducible Performance Checkpoint compiler.
...
from string_performance_critic_v48 import generate_repairs_v48,critic_bundle_dict,evaluate_performance_v48
...
from conductor_candidate_steering_v54 import steer_candidates_v54
from performance_archetype_memory_v58 import classify_archetype_v58
from archetype_mixture_v59 import soft_classify_archetype_v59

def _retag(path:Path):
    b=path.read_bytes()
    for old in (b"v4.7",b"v4.8"):
        b=b.replace(old,b"v6.2")
    path.write_bytes(b)
...
def compile_file(src:Path,out:Path|None=None,policy_path:Path|None=None,round_index:int=1):
    ...
    archetype=classify_archetype_v58(conductor_intent)
    archetype_mixture=soft_classify_archetype_v59(conductor_intent)
    score,issues,candidates,reports,recommended_pre_steer=generate_repairs_v48(g,policy=snap.values)
    candidates,steering_report=steer_candidates_v54(g,candidates,conductor_intent)
    steered_scores={slot:evaluate_performance_v48(candidates[slot])[0].overall for slot in "ABC"}
    recommended=max("ABC",key=lambda slot:steered_scores[slot])

    out=out or src.with_name(src.stem+f"_SONICRAFT_STRINGS_v62_R{round_index}.mid")
    out=Path(out)
    ...
    steering_json=out.with_suffix(".candidate_steering.json")
    archetype_json=out.with_suffix(".performance_archetype.json")
    mixture_json=out.with_suffix(".performance_archetype_mixture.json")
    ...
    steering_json.write_text(json.dumps(steering_report.as_dict(),ensure_ascii=False,indent=2),encoding="utf-8")
    archetype_json.write_text(json.dumps({
        "schema":1,"version":"6.1",
        "classification":archetype.as_dict(),
        "privacy_note":"legacy hard-label view retained for compatibility; D-derived aggregate controls only",
    },ensure_ascii=False,indent=2),encoding="utf-8")
    mixture_json.write_text(json.dumps({
        "schema":1,"version":"6.1",
        "soft_mixture":archetype_mixture.as_dict(),
        "hard_limits":{"max_components":3,"min_component_weight":.08,
                       "mixture_only_top1_forbidden":True,"low_fit_block":.42},
        "privacy_note":"D-derived aggregate control mixture only; no song identity, note sequence, score text, MIDI or audio",
    },ensure_ascii=False,indent=2),encoding="utf-8")

    data=graph_dict(g)
    data.update({
        "sonicraft_version":"6.1",
        ...
        "candidate_steering":{
            "sidecar":steering_json.name,
            "intent_hash":steering_report.intent_hash,
            "steered_scores":steered_scores,
            "pre_steer_structural_recommendation":recommended_pre_steer,
            "post_steer_structural_recommendation":recommended,
        },
        "performance_archetype":{
            "label":archetype.label,
            "confidence":archetype.confidence,
            "secondary_label":archetype.secondary_label,
            "secondary_confidence":archetype.secondary_confidence,
            "sidecar":archetype_json.name,
            "control_profile_only":True,
        },
        "performance_archetype_mixture":{
            "components":[{"label":c.label,"weight":c.weight,"distance":c.distance} for c in archetype_mixture.components],
            "confidence":archetype_mixture.confidence,
            "weighted_distance":archetype_mixture.weighted_distance,
            "entropy":archetype_mixture.entropy,
            "sidecar":mixture_json.name,
            "mixture_only_top1_forbidden":True,
        },
        "repair_policy":{
    ...
    gesture_json.write_text(json.dumps({"schema":1,"version":"6.1",
    ...
    cb.update({
        "version":"6.1","round_index":round_index,
    ...
    queue={
        ...
        "purpose":"render A/B/C/D identically, then run v6.2 post-render iteration",
        "slots":{
        ...
        "conductor_intent_lock":{"supported":True,"intent_hash":conductor_intent.intent_hash,
                                 ...
                                 "sidecar":conductor_json.name},
        "conductor_steered_generation":{"supported":True,"sidecar":steering_json.name,
                                        "intent_hash":steering_report.intent_hash,
                                        "progressive_candidate_budget":True,
                                        "deferred_candidate_escalation":True,
                                        "D_original_never_steered":True,
                                        "active_slot_policy":steering_report.active_slot_policy},
        "candidate_utility_predictor":{"supported":True,"actual_render_only_learning":True,
                                       "D_always_rendered":True,"low_confidence_uses_v54_budget":True,
                                       "predictor_audio_disagreement_escalates":True,
                                       "zero_render_pruning":True},
        "counterfactual_render_auditor":{"supported":True,"base_audit_interval":12,
                                          "elevated_audit_interval":6,"high_risk_audit_interval":4,
                                          "false_prune_margin":.025,"context_disable_rate":.25,
                                          "recovery_clean_audits":4,"actual_render_only":True,
                                          "zero_render_can_be_disabled_per_context":True,
                                          "no_audio_midi_score_storage":True},
        "context_similarity_transfer":{"supported":True,"same_section_character_only":True,
                                        "critic_dimension_overlap_required":True,"min_jaccard":.34,
                                        "transfer_evidence_discounted":True,
                                        "transfer_only_top1_pruning_forbidden":True,
                                        "target_local_evidence_required_for_top1":1.5,
                                        "donor_audit_risk_blocks_transfer":True,
                                        "edge_specific_counterfactual_calibration":True,
                                        "no_audio_midi_score_storage":True},
        "cross_song_archetype_memory":{"supported":True,
                                      "label":archetype.label,
                                      "classification_confidence":archetype.confidence,
                                      "control_profile_only":True,
                                      "archetype_only_top1_forbidden":True,
                                      "audit_calibrated":True,
                                      "sidecar":archetype_json.name},
        "soft_archetype_mixture":{"supported":True,
                                  "components":[{"label":c.label,"weight":c.weight} for c in archetype_mixture.components],
                                  "mixture_confidence":archetype_mixture.confidence,
                                  "max_components":3,"min_component_weight":.08,
                                  "mixture_only_top1_forbidden":True,
                                  "component_edge_audit_calibration":True,
                                  "sidecar":mixture_json.name},
        "unified_evidence_store":{"supported":True,"schema":1,
                                  "namespaces":["utility_v55","audit_v56","similarity_v57","archetype_v58","mixture_v59"],
                                  "content_addressed":True,"atomic_multi_namespace_commit":True,
                                  "rollback":True,"quarantine":True,"export_import":True,
                                  "legacy_json_compatible":True},
        "reproducible_performance_checkpoint":{
            "supported":True,"schema":1,
            "binds":["source_score_sha256","compiler_code_fingerprint","evidence_input_commit",
                     "repair_policy_snapshot","conductor_intent_hash","compiled_artifact_hashes",
                     "acoustic_runtime_binding_sha256","model_weight_sha256","renderer_build_sha256",
                     "backend_framework_version","device_capability","sample_rate_render_config"],
            "non_destructive_replay_verify":True,
            "explicit_environment_restore":True,
            "acoustic_runtime_provenance":True,
            "audio_embedded":False,
            "exact_audio_replay_claimed":False
        },
        "next_command":"AUTO_LOOP_STRINGS_v62.bat <source score>",
    }
    ...
    return {
        ...
        "conductor_json":conductor_json,"steering_json":steering_json,
        "archetype_json":archetype_json,"archetype":archetype,
        "mixture_json":mixture_json,"archetype_mixture":archetype_mixture,
        "judge_queue_json":judge_queue_json,
        "graph":g,"score":score,"reports":reports,
        "recommended":recommended,"recommended_pre_steer":recommended_pre_steer,
        "steered_scores":steered_scores,"steering_report":steering_report,
        "policy_snapshot":snap,
        "candidate_graphs":candidates,"issues":issues,"conductor_intent":conductor_intent,
    }

def main(argv=None):
    ap=argparse.ArgumentParser(description="SONICRAFT v6.2 Reproducible Performance Checkpoint Strings compiler.")
    ...
    print("SONICRAFT v6.2 Reproducible Performance Checkpoint Compile OK")
