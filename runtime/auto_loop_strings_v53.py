"""SONICRAFT v5.3 Long-Form Conductor Intent / Section Character Lock Auto-Loop.
...
from compile_musicxml_strings_v53 import compile_file
...
def _copy_final(src_score,out_dir,midi,wav,label):
    ...
    mo=out_dir/f"{stem}_SONICRAFT_STRINGS_v53_{label}.mid"
    wo=out_dir/f"{stem}_SONICRAFT_STRINGS_v53_{label}.wav"
...
def run_auto_loop_v53(score,out_dir=None,policy_path=None,host="127.0.0.1",port=49337,
    ...
    out_dir=Path(out_dir) if out_dir else score.with_name(score.stem+"_SONICRAFT_v53_SELECTIVE")
    ...
    try:
        ...
        for round_index in range(1,max_round+1):
            out=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v53_R{round_index}.mid")
            ...
            conductor_path=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v53_R{round_index}.conductor_lock.json")
            ...
            merged=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v53_R{round_index}_SELECTIVE_MERGED.mid")
            ...
            d_verify=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v53_R{round_index}_D_GLOBAL_VERIFY.wav")
    ...
    report={
        ...
        "authority_note":"Local Audio Judge proposes phrase repairs. Global Coherence protects continuity; Long-Form Conductor Intent locks macro section character/climax/roles and may substitute only near-scoring candidates. Final merged-vs-D pair verification protects whole-song audio.",
    }
    rp=out_dir/(score.stem+"_SONICRAFT_STRINGS_v53_DECISION_TRACE.json")
    rp.write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding="utf-8")
    return report,rp

def main(argv=None):
    ap=argparse.ArgumentParser(description="SONICRAFT v5.3 Long-Form Conductor Intent / Section Character Lock auto-loop.")
    ...
    try:
        r,rp=run_auto_loop_v53(a.score,a.out_dir,a.policy,a.host,a.port,a.backend,a.model_dir,a.cache_dir,
    ...
    print("SONICRAFT v5.3 Conductor Intent Auto-Loop finished")
