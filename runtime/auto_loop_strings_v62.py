"""SONICRAFT v6.2 Reproducible Performance Checkpoint Auto-Loop.
...
from performance_checkpoint_v62 import create_compile_checkpoint_v62,finalize_checkpoint_v62
from acoustic_runtime_provenance_v62 import capture_acoustic_runtime_provenance_v62
...
def _copy_final(src_score,out_dir,midi,wav,label):
    ...
    mo=out_dir/f"{stem}_SONICRAFT_STRINGS_v62_{label}.mid"
    wo=out_dir/f"{stem}_SONICRAFT_STRINGS_v62_{label}.wav"
...
def run_auto_loop_v62(score,out_dir=None,policy_path=None,host="127.0.0.1",port=49337,
    ...
    out_dir=Path(out_dir) if out_dir else score.with_name(score.stem+"_SONICRAFT_v62_SELECTIVE")
    out_dir.mkdir(parents=True,exist_ok=True)
    checkpoint_dir=Path(checkpoint_dir) if checkpoint_dir else (out_dir/"CHECKPOINTS")
    ...
    # Capture once per Auto-Loop invocation. Hashing multi-GB weights once is authoritative without
    # multiplying round latency. Every round checkpoint binds this same acoustic execution context.
    acoustic_runtime=capture_acoustic_runtime_provenance_v62(
        model_dir=model_dir,backend=backend,mock=mock,sample_rate=sample_rate,
        chunk_seconds=chunk_seconds,overlap_seconds=overlap_seconds,
        local_context=local_context,max_local_context_seconds=max_local_context_seconds
    )
    ...
    try:
        ...
        for round_index in range(1,max_round+1):
            out=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v62_R{round_index}.mid")
            comp=compile_file(score,out,policy_path,round_index)
            checkpoint_path=checkpoint_dir/(score.stem+f"_R{round_index:02d}.performance_checkpoint.json")
            checkpoint_payload,checkpoint_path=create_compile_checkpoint_v62(
                comp,score,evidence_store,round_index,checkpoint_path,acoustic_runtime=acoustic_runtime
            )
            ...
            if not plan.selective:
                ...
                cp=finalize_checkpoint_v62(
                    checkpoint_path,evidence_store,q["policy_path"],
                    decision_summary={"mode":"full_fallback","winner":row["winner"],"runner_up":row["runner_up"],
                                      "margin":row["margin"],"reason":row["fallback_reason"]},
                    artifacts={"winner_midi":out_dir/qfull["slots"][row["winner"]]["midi"]},
                    status="full_fallback_round_complete"
                )
                row.update({"round":round_index,"plan":plan.as_dict(),
                            "performance_checkpoint":str(checkpoint_path),
                            "checkpoint_id":cp["checkpoint_id"],
                            "checkpoint_result_binding":cp.get("result_binding_sha256")})
                trace.append(row)
                if not learned:
            ...
            if low_conf:
                ...
                cp=finalize_checkpoint_v62(
                    checkpoint_path,evidence_store,q["policy_path"],
                    decision_summary={"mode":"full_fallback_after_local","winner":row["winner"],
                                      "runner_up":row["runner_up"],"margin":row["margin"],
                                      "reason":low_conf,"local_decision_count":len(decisions)},
                    artifacts={"winner_midi":out_dir/qfull["slots"][row["winner"]]["midi"]},
                    status="local_ambiguity_fallback_complete"
                )
                row.update({"round":round_index,"plan":plan.as_dict(),
            ...
            utility_path=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v62_R{round_index}.candidate_utility.json")
            utility_path.write_text(json.dumps({
                "schema":1,"version":"6.1","memory_path":str(utility_mem.path),
            ...
            audit_path=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v62_R{round_index}.counterfactual_audit.json")
            audit_path.write_text(json.dumps({
                "schema":1,"version":"6.1","memory_path":str(audit_mem.path),
            ...
            transfer_path=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v62_R{round_index}.context_similarity_transfer.json")
            transfer_path.write_text(json.dumps({
                "schema":1,"version":"6.1","memory_path":str(transfer_mem.path),
            ...
            archetype_path=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v62_R{round_index}.performance_archetype_memory.json")
            archetype_path.write_text(json.dumps({
                "schema":1,"version":"6.1","memory_path":str(archetype_mem.path),
            ...
            mixture_path=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v62_R{round_index}.archetype_mixture_memory.json")
            mixture_path.write_text(json.dumps({
                "schema":1,"version":"6.1","memory_path":str(mixture_mem.path),
            ...
            conductor_path=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v62_R{round_index}.conductor_lock.json")
            ...
            merged=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v62_R{round_index}_SELECTIVE_MERGED.mid")
            ...
            if learn.get("learned") and round_index<max_round:
                cp=finalize_checkpoint_v62(
                    checkpoint_path,evidence_store,q["policy_path"],
                    decision_summary={
                        "mode":"selective_learning_continue",
                        "window_winners":[{"window_id":d["window_id"],"winner":d["winner"],
                                           "margin":d["margin"]} for d in decisions],
                        "policy_learning":row["learning"],
                        "global_coherence_score":row["global_coherence"].get("score"),
                        "conductor_intent_hash":row["conductor_steering"]["intent_hash"],
                    },
                    artifacts={"merged_midi":merged,"conductor_lock":conductor_path},
                    status="selective_round_learned_continue"
                )
                row["checkpoint_result_binding"]=cp.get("result_binding_sha256")
                continue
            ...
            d_verify=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v62_R{round_index}_D_GLOBAL_VERIFY.wav")
            ...
            if not pair_pass:
                ...
                cp=finalize_checkpoint_v62(
                    checkpoint_path,evidence_store,q["policy_path"],
                    decision_summary={
                        "mode":"global_pair_fallback","winner":row2["winner"],"margin":row2["margin"],
                        "pair_overall_delta":pair_delta,"pair_safety_delta":pair_safety_delta,
                        "window_winners":[{"window_id":d["window_id"],"winner":d["winner"]} for d in decisions],
                    },
                    artifacts={"merged_midi":merged,"fallback_winner_midi":midi},
                    status="global_pair_fallback_complete"
                )
                row["checkpoint_result_binding"]=cp.get("result_binding_sha256")
                final["performance_checkpoint"]=str(checkpoint_path)
                final["checkpoint_id"]=cp["checkpoint_id"]
                break
            ...
            cp=finalize_checkpoint_v62(
                checkpoint_path,evidence_store,q["policy_path"],
                decision_summary={
                    "mode":"selective_conductor_lock",
                    "status":final["status"],
                    "window_winners":final["window_winners"],
                    "pair_verify":final["pair_verify"],
                    "global_coherence_score":final["global_coherence"].get("score"),
                    "conductor_intent_hash":final["conductor_intent"].get("intent_hash"),
                },
                artifacts={"merged_midi":merged,"final_midi":fm},
                status="selective_final_verified"
            )
            row["checkpoint_result_binding"]=cp.get("result_binding_sha256")
            final["performance_checkpoint"]=str(checkpoint_path)
            final["checkpoint_id"]=cp["checkpoint_id"]
            final["checkpoint_result_binding"]=cp.get("result_binding_sha256")
            break
    ...
    report={
        "schema":1,"version":"6.1","source_score":str(score),"out_dir":str(out_dir.resolve()),
        ...
        "reproducible_performance_checkpoint":{
            "directory":str(checkpoint_dir.resolve()),"schema":1,
            "input_state_binds_evidence_and_policy":True,
            "compiler_code_fingerprint":True,
            "conductor_intent_hash":True,
            "artifact_hashes":True,
            "acoustic_runtime_binding":acoustic_runtime["binding_sha256"],
            "model_environment_bound":True,"renderer_build_bound":True,
            "backend_framework_bound":True,"device_capability_bound":True,
            "sample_rate_render_config_bound":True,
            "non_destructive_replay_verify":True,
            "explicit_restore_only":True,
            "exact_audio_replay_claimed":False},
        "authority_note":"v6.2 binds v6.1's deterministic Score/Control/Policy/Evidence/Compiler environment plus model weights/manifests, renderer build, selected backend/framework, runtime/device capability and sample-rate/render configuration. It can explain acoustic-environment drift but still does not claim bit-identical audio replay.",
    }
    rp=out_dir/(score.stem+"_SONICRAFT_STRINGS_v62_DECISION_TRACE.json")
    rp.write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding="utf-8")
    return report,rp

def main(argv=None):
    ap=argparse.ArgumentParser(description="SONICRAFT v6.2 Reproducible Performance Checkpoint auto-loop.")
    ...
    try:
        r,rp=run_auto_loop_v62(a.score,a.out_dir,a.policy,a.host,a.port,a.backend,a.model_dir,a.cache_dir,
                               ...
                               a.evidence_store,a.checkpoint_dir)
    except Exception as ex:
        print("ERROR:",ex,file=sys.stderr);return 2
    print("SONICRAFT v6.2 Reproducible Performance Checkpoint Auto-Loop finished")
