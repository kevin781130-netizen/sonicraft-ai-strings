"""SONICRAFT v4.7 Phrase-Level Bow Energy & Vibrato Continuity Compiler.
...
from string_phrase_longline_v47 import plan_phrase_longlines_v47,phrase_graph_dict,SENTINEL_NORM
...
def _gesture_events_v47(n,ch):
    ...
    if not n.transition_in_link_id:
        if n.phrase_longline_enabled:
            out.append((pre,1,bytes([0xB0|ch,38,1])))  # v4.7 backward-safe sentinel
...
def write_midi(g,constraints,ensemble,links,out:Path):
    conductor=[(0,0,_name_event('SONICRAFT v4.7 Phrase Long-Line Strings'))]
    ...
    for p,name in enumerate(PARTS):
        ev=[(0,0,_name_event('SONICRAFT '+name+' · v4.7 Phrase Long-Line Voice Bus'))]
...
def compile_file(src:Path,out:Path|None=None,score_json:Path|None=None,constraints_json:Path|None=None,ensemble_json:Path|None=None,gesture_json:Path|None=None,transition_json:Path|None=None,phrase_json:Path|None=None):
    ...
    out=out or src.with_name(src.stem+'_SONICRAFT_STRINGS_v47.mid')
    ...
    phrase_json=phrase_json or out.with_suffix('.phrase.json')
    ...
    data.update({
        'sonicraft_version':'4.7','compiled_midi':out.name,
        ...
        'phrase_longline':{
            'phrase_count':len(phrase_arcs),'new_midi_cc_or_paramids':False,
            'activation':'CC38 1/127 sentinel + existing phrase window',
            'bow_energy_arc':True,'vibrato_rate_target':True,'dynamic_momentum':True,
            'legacy_v46_without_sentinel_unchanged':True,
        },
        'physical_notes':[{'source_id':n.source_id,**physical_note_dict(n),
                           ...
                           'transition_flags':list(n.transition_flags),'phrase_longline_id':n.phrase_longline_id,'phrase_longline_contour':n.phrase_longline_contour,'phrase_bow_reserve':round(float(n.phrase_bow_reserve),6),'phrase_dynamic_momentum':round(float(n.phrase_dynamic_momentum),6),'phrase_vibrato_rate_hz':round(float(n.phrase_vibrato_rate_hz),6),'phrase_longline_flags':list(n.phrase_longline_flags)} for n in g.notes],
    ...
    gesture_json.write_text(json.dumps({'schema':1,'version':'4.7','gesture_notes':gestures},ensure_ascii=False,indent=2),encoding='utf-8')
    ...
    return out,score_json,constraints_json,ensemble_json,gesture_json,transition_json,phrase_json,g,constraints,ensemble,links,phrase_arcs

def main(argv=None):
    ap=argparse.ArgumentParser(description='Compile MusicXML/MXL with SONICRAFT v4.7 Phrase-Level Bow Energy & Vibrato Continuity.')
    ap.add_argument('score',type=Path);ap.add_argument('-o','--out',type=Path);a=ap.parse_args(argv)
    try:o,s,c,e,gj,tj,pj,g,cr,er,links,arcs=compile_file(a.score,a.out)
    except Exception as ex:print('ERROR:',ex,file=sys.stderr);return 2
    print('SONICRAFT v4.7 Phrase Long-Line Compile OK')
    print('MIDI:',o);print('Transition:',tj);print('Phrase:',pj);print('notes:',len(g.notes),'links:',len(links),'phrases:',len(arcs));return 0
