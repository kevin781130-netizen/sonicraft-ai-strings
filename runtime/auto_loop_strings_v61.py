"""SONICRAFT v6.1 Reproducible Performance Checkpoint Auto-Loop.
...
from performance_checkpoint_v61 import create_compile_checkpoint_v61,finalize_checkpoint_v61
...
def _copy_final(src_score,out_dir,midi,wav,label):
    ...
    mo=out_dir/f"{stem}_SONICRAFT_STRINGS_v61_{label}.mid"
    wo=out_dir/f"{stem}_SONICRAFT_STRINGS_v61_{label}.wav"
...
def run_auto_loop_v61(score,out_dir=None,policy_path=None,host="127.0.0.1",port=49337,
    ...
    out_dir=Path(out_dir) if out_dir else score.with_name(score.stem+"_SONICRAFT_v61_SELECTIVE")
    ...
    try:
        ...
        for round_index in range(1,max_round+1):
            out=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v61_R{round_index}.mid")
            ...
            checkpoint_payload,checkpoint_path=create_compile_checkpoint_v61(
                comp,score,evidence_store,round_index,checkpoint_path
            )
            ...
            utility_path=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v61_R{round_index}.candidate_utility.json")
            ...
            audit_path=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v61_R{round_index}.counterfactual_audit.json")
            ...
            transfer_path=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v61_R{round_index}.context_similarity_transfer.json")
            ...
            archetype_path=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v61_R{round_index}.performance_archetype_memory.json")
            ...
            mixture_path=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v61_R{round_index}.archetype_mixture_memory.json")
            ...
            conductor_path=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v61_R{round_index}.conductor_lock.json")
            ...
            merged=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v61_R{round_index}_SELECTIVE_MERGED.mid")
            ...
            d_verify=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v61_R{round_index}_D_GLOBAL_VERIFY.wav")
    ...
    report={
        ...
        "authority_note":"v6.1 binds each round's pre-decision Evidence HEAD, exact Repair Policy snapshot, compiler code fingerprint, source score SHA, Conductor Intent and deterministic compile artifact hashes into a project-local checkpoint. It does not alter v5.5-v5.9 evidence semantics, and Replay Verify does not claim acoustic/audio determinism.",
    }
    rp=out_dir/(score.stem+"_SONICRAFT_STRINGS_v61_DECISION_TRACE.json")
    rp.write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding="utf-8")
    return report,rp

def main(argv=None):
    ap=argparse.ArgumentParser(description="SONICRAFT v6.1 Reproducible Performance Checkpoint auto-loop.")
    ...
    try:
        r,rp=run_auto_loop_v61(a.score,a.out_dir,a.policy,a.host,a.port,a.backend,a.model_dir,a.cache_dir,
    ...
    print("SONICRAFT v6.1 Reproducible Performance Checkpoint Auto-Loop finished")
