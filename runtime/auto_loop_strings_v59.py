"""SONICRAFT v5.9 Multi-Archetype Mixture / Soft Classification Auto-Loop.
...
from compile_musicxml_strings_v59 import compile_file
...
def _copy_final(src_score,out_dir,midi,wav,label):
    ...
    mo=out_dir/f"{stem}_SONICRAFT_STRINGS_v59_{label}.mid"
    wo=out_dir/f"{stem}_SONICRAFT_STRINGS_v59_{label}.wav"
...
def run_auto_loop_v59(score,out_dir=None,policy_path=None,host="127.0.0.1",port=49337,
                      ...
                      archetype_memory_path=None,mixture_memory_path=None):
    score=Path(score).resolve()
    if not score.exists():raise FileNotFoundError(score)
    out_dir=Path(out_dir) if out_dir else score.with_name(score.stem+"_SONICRAFT_v59_SELECTIVE")
    ...
    mixture_mem=ArchetypeMixtureMemoryV59(mixture_memory_path or default_mixture_path_v59(utility_mem.path))
    ...
    try:
        ...
        for round_index in range(1,max_round+1):
            out=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v59_R{round_index}.mid")
            ...
            utility_path=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v59_R{round_index}.candidate_utility.json")
            utility_path.write_text(json.dumps({
                "schema":1,"version":"5.9","memory_path":str(utility_mem.path),
            ...
            audit_path=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v59_R{round_index}.counterfactual_audit.json")
            audit_path.write_text(json.dumps({
                "schema":1,"version":"5.9","memory_path":str(audit_mem.path),
            ...
            transfer_path=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v59_R{round_index}.context_similarity_transfer.json")
            transfer_path.write_text(json.dumps({
                "schema":1,"version":"5.9","memory_path":str(transfer_mem.path),
            ...
            archetype_path=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v59_R{round_index}.performance_archetype_memory.json")
            archetype_path.write_text(json.dumps({
                "schema":1,"version":"5.9","memory_path":str(archetype_mem.path),
            ...
            mixture_path=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v59_R{round_index}.archetype_mixture_memory.json")
            mixture_path.write_text(json.dumps({
                "schema":1,"version":"5.9","memory_path":str(mixture_mem.path),
            ...
            conductor_path=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v59_R{round_index}.conductor_lock.json")
            ...
            merged=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v59_R{round_index}_SELECTIVE_MERGED.mid")
            ...
            d_verify=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v59_R{round_index}_D_GLOBAL_VERIFY.wav")
    ...
    report={
        "schema":1,"version":"5.9","source_score":str(score),"out_dir":str(out_dir.resolve()),
        ...
        "authority_note":"v5.9 replaces hard single-archetype borrowing with a D-derived soft mixture of up to three control prototypes. Mixture-only evidence can at most unlock Top2+D; target-local evidence remains mandatory for Top1+D. Counterfactual failures calibrate only mixture component->context edges, while v5.5 Utility, v5.7 transfer edges and v5.8 archetype trust remain isolated.",
    }
    rp=out_dir/(score.stem+"_SONICRAFT_STRINGS_v59_DECISION_TRACE.json")
    rp.write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding="utf-8")
    return report,rp

def main(argv=None):
    ap=argparse.ArgumentParser(description="SONICRAFT v5.9 Context Generalization / Similarity Transfer auto-loop.")
    ...
    try:
        r,rp=run_auto_loop_v59(a.score,a.out_dir,a.policy,a.host,a.port,a.backend,a.model_dir,a.cache_dir,
    ...
    print("SONICRAFT v5.9 Multi-Archetype Mixture Auto-Loop finished")
