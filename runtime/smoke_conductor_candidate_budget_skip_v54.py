...
with tempfile.TemporaryDirectory() as td:
    ...
    assert any(d["candidate_budget"]["deferred_slots"] for d in row["decisions"]),row["decisions"]
    for d in row["decisions"]:
        if d["candidate_budget"]["deferred_slots"]:
            assert not d["candidate_budget"]["expanded"],d
            for s in d["candidate_budget"]["deferred_slots"]:
                assert s not in d["local_wavs"],d
    ...
    print("SONICRAFT v5.4 progressive candidate budget SKIP smoke OK",
