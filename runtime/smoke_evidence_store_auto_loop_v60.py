...
import numpy as np
import auto_loop_strings_v60 as loop
from compile_musicxml_strings_v60 import compile_file as real_compile
...
with tempfile.TemporaryDirectory() as td:
    ...
    utility=td/"utility.json";audit=td/"audit.json";transfer=td/"transfer.json";archp=td/"archetype_memory.json";mixp=td/"mixture_memory.json";storep=td/"evidence_store.json"
    ...
    def wrapped_compile(*args,**kwargs):
        ...
        r["archetype_json"].write_text(json.dumps({"schema":1,"version":"6.0","classification":cls.as_dict()},indent=2))
        r["mixture_json"].write_text(json.dumps({"schema":1,"version":"6.0","soft_mixture":mix.as_dict()},indent=2))
        return r
    ...
    try:
        report,rp=loop.run_auto_loop_v60(
            score,td/"out",td/"policy.json",port=49623,mock=True,max_round=1,
            ...
            archetype_memory_path=archp,mixture_memory_path=mixp,evidence_store_path=storep
        )
    finally:
    ...
    print("SONICRAFT v6.0 transactional Auto-Loop + startup drift recovery smoke OK",
          ...
          "mix_ev",cb["mixture_evidence"],"store_commits",st["commits"],"quarantine",estore.status()["quarantine"],
