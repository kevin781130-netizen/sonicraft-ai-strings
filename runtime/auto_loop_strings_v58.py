"""SONICRAFT v5.8 Context Generalization / Similarity Transfer Auto-Loop.
...
from compile_musicxml_strings_v58 import compile_file
...
from performance_archetype_memory_v58 import PerformanceArchetypeMemoryV58,predict_candidate_utility_v58,default_archetype_path_v58
...
def _copy_final(src_score,out_dir,midi,wav,label):
    ...
    mo=out_dir/f"{stem}_SONICRAFT_STRINGS_v58_{label}.mid"
    wo=out_dir/f"{stem}_SONICRAFT_STRINGS_v58_{label}.wav"
...
def run_auto_loop_v58(score,out_dir=None,policy_path=None,host="127.0.0.1",port=49337,
                      ...
                      archetype_memory_path=None):
    score=Path(score).resolve()
    if not score.exists():raise FileNotFoundError(score)
    out_dir=Path(out_dir) if out_dir else score.with_name(score.stem+"_SONICRAFT_v58_SELECTIVE")
    out_dir.mkdir(parents=True,exist_ok=True)
    utility_mem=CandidateUtilityMemoryV55(utility_memory_path or default_utility_path(policy_path))
    audit_mem=CounterfactualAuditMemoryV56(audit_memory_path or default_audit_path_v56(utility_mem.path))
    transfer_mem=SimilarityTransferMemoryV57(transfer_memory_path or default_transfer_path_v57(utility_mem.path))
    archetype_mem=PerformanceArchetypeMemoryV58(archetype_memory_path or default_archetype_path_v58(utility_mem.path))
    ...
    try:
        ...
        for round_index in range(1,max_round+1):
            out=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v58_R{round_index}.mid")
            ...
            for wi,w in enumerate(plan.windows):
                ...
                prediction=predict_candidate_utility_v58(
                    ...
                    archetype_mem,comp["archetype"],v54_budget["active"]
                )
                ...
                if escalate and deferred:
                    ...
                    if len(rendered)==4 and len(preaudit_scores)>=2:
                        ...
                        if prediction.archetype_evidence>0:
                            arche_audit=archetype_mem.record_audit(
                                prediction.archetype_label,prediction.context_key,audit_record
                            )
                            if arche_audit.get("recorded"):archetype_audit_events.append(arche_audit)
                else:
                ...
                if accepted_local:
                    ...
                    arche_update=archetype_mem.learn_rendered(
                        comp["archetype"].label,prediction.context_key,rendered_score_dict,winner,
                        comp["archetype"].confidence,full_evidence=(len(rendered)==4)
                    )
                ...
                d={
                    ...
                    "candidate_budget":{
                        ...
                        "archetype_label":prediction.archetype_label,
                        "archetype_classification_confidence":prediction.archetype_classification_confidence,
                        "archetype_evidence":prediction.archetype_evidence,
                        "archetype_confidence":prediction.archetype_confidence,
                        "archetype_detail":prediction.archetype_detail,
            ...
            utility_path=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v58_R{round_index}.candidate_utility.json")
            utility_path.write_text(json.dumps({
                "schema":1,"version":"5.8","memory_path":str(utility_mem.path),
            ...
            audit_path=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v58_R{round_index}.counterfactual_audit.json")
            audit_path.write_text(json.dumps({
                "schema":1,"version":"5.8","memory_path":str(audit_mem.path),
            ...
            transfer_path=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v58_R{round_index}.context_similarity_transfer.json")
            transfer_path.write_text(json.dumps({
                "schema":1,"version":"5.8","memory_path":str(transfer_mem.path),
            ...
            archetype_path=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v58_R{round_index}.performance_archetype_memory.json")
            archetype_path.write_text(json.dumps({
                "schema":1,"version":"5.8","memory_path":str(archetype_mem.path),
                "memory_generation":archetype_mem.generation,
                "classification":comp["archetype"].as_dict(),
                "predictions":[{"context_key":p.get("context_key"),"archetype_label":p.get("archetype_label"),
                                "archetype_classification_confidence":p.get("archetype_classification_confidence"),
                                "archetype_evidence":p.get("archetype_evidence"),
                                "archetype_confidence":p.get("archetype_confidence"),
                                "archetype_detail":p.get("archetype_detail"),
                                "reason":p.get("reason")} for p in utility_predictions],
                "updates":archetype_updates,"audit_edge_updates":archetype_audit_events,
                "hard_isolation":{"archetype_only_top1_forbidden":True,
                                  "actual_render_only_learning":True,
                                  "low_classification_confidence_blocks":True},
            ...
            conductor_path=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v58_R{round_index}.conductor_lock.json")
            ...
            merged=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v58_R{round_index}_SELECTIVE_MERGED.mid")
            ...
            d_verify=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v58_R{round_index}_D_GLOBAL_VERIFY.wav")
    ...
    report={
        "schema":1,"version":"5.8","source_score":str(score),"out_dir":str(out_dir.resolve()),
        ...
        "cross_song_performance_archetype":{
            ...
            "actual_render_only_learning":True,"audit_calibrated":True,
            "labels":["intimate","ballad","dramatic","chamber","cinematic"]},
        "authority_note":"v5.8 adds discounted cross-song performance-archetype evidence on top of exact Utility and v5.7 Similarity Transfer. Archetype-only evidence can at most unlock Top2+D; actual target-context evidence is still required for Top1+D. False-prune audits calibrate archetype->context trust without mutating Utility or transfer donors. Audio Judge and all downstream guards remain authoritative.",
    }
    rp=out_dir/(score.stem+"_SONICRAFT_STRINGS_v58_DECISION_TRACE.json")
    rp.write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding="utf-8")
    return report,rp

def main(argv=None):
    ap=argparse.ArgumentParser(description="SONICRAFT v5.8 Context Generalization / Similarity Transfer auto-loop.")
    ...
    try:
        r,rp=run_auto_loop_v58(a.score,a.out_dir,a.policy,a.host,a.port,a.backend,a.model_dir,a.cache_dir,
                               ...
                               a.utility_memory,a.audit_memory,a.transfer_memory,a.archetype_memory)
    except Exception as ex:
        print("ERROR:",ex,file=sys.stderr);return 2
    print("SONICRAFT v5.8 Cross-Song Performance Archetype Auto-Loop finished")
