"""SONICRAFT v5.8 Cross-Song Performance Archetype Memory compiler.
...
def compile_file(src:Path,out:Path|None=None,policy_path:Path|None=None,round_index:int=1):
    ...
    out=out or src.with_name(src.stem+f"_SONICRAFT_STRINGS_v58_R{round_index}.mid")
    out=Path(out)
    ...
    archetype_json.write_text(json.dumps({
        "schema":1,"version":"5.8",
        "classification":archetype.as_dict(),
        "privacy_note":"project sidecar contains D-derived aggregate performance-control features only; persistent archetype memory stores no song identity, note sequence, score text or audio",
    },ensure_ascii=False,indent=2),encoding="utf-8")

    data=graph_dict(g)
    data.update({
        "sonicraft_version":"5.8",
    ...
    gesture_json.write_text(json.dumps({"schema":1,"version":"5.8",
    ...
    cb.update({
        "version":"5.8","round_index":round_index,
    ...
    policy_json.write_text(json.dumps({
        "schema":1,"version":"5.8","generation":snap.generation,"evidence":snap.evidence,
        "confidence":snap.confidence,"profile_hash":snap.profile_hash,"values":snap.values,
        "persistent_policy_path":str(mem.path),
    },ensure_ascii=False,indent=2),encoding="utf-8")

    queue={
        "schema":2,"version":"5.8","round_index":round_index,
        ...
        "purpose":"render A/B/C/D identically, then run v5.8 post-render iteration",
        "slots":{
        ...
        "cross_song_archetype_memory":{"supported":True,
                                      ...
                                      "sidecar":archetype_json.name},
        "next_command":"AUTO_LOOP_STRINGS_v58.bat <source score>",
    }
...
def main(argv=None):
    ap=argparse.ArgumentParser(description="SONICRAFT v5.8 Context Generalization / Similarity Transfer Strings compiler.")
    ...
    print("SONICRAFT v5.8 Context Similarity Transfer Compile OK")
