"""SONICRAFT v5.4 Conductor-Steered Candidate Generation Auto-Loop.
...
def _copy_final(src_score,out_dir,midi,wav,label):
    ...
    mo=out_dir/f"{stem}_SONICRAFT_STRINGS_v54_{label}.mid"
    wo=out_dir/f"{stem}_SONICRAFT_STRINGS_v54_{label}.wav"
...
def run_auto_loop_v54(score,out_dir=None,policy_path=None,host="127.0.0.1",port=49337,
    ...
    out_dir=Path(out_dir) if out_dir else score.with_name(score.stem+"_SONICRAFT_v54_SELECTIVE")
    ...
    try:
        ...
        for round_index in range(1,max_round+1):
            out=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v54_R{round_index}.mid")
            ...
            for wi,w in enumerate(plan.windows):
                ...
                budget=render_slots_for_window_v54(comp["conductor_intent"],w.start_tick,w.end_tick)
                active=list(budget["active"]);deferred=list(budget["deferred"])
                ...
                def render_slot(slot,serial):
                    ...
                    rr=render_midi_window_v51(
                        ...
                        request_id=5540000+round_index*10000+wi*100+serial*10,
                ...
                for si,slot in enumerate(active):
                    render_slot(slot,si)
                ...
                # Progressive budget: only pay for deferred candidates when the initial set
                # cannot produce a confident local decision.
                if margin<MIN_MARGIN and deferred:
                    expanded=True
                ...
                d={
                    ...
                    "candidate_budget":{
                        "section_id":budget["section_id"],"character":budget["character"],
                        "primary_slots":active,"deferred_slots":deferred,
                        "rendered_slots":list(rendered),"expanded":expanded,
                        "initial_margin":round(initial_margin,9),
                        "final_margin":round(margin,9),
                    },
                }
                decisions.append(d)
            ...
            conductor_path=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v54_R{round_index}.conductor_lock.json")
            ...
            merged=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v54_R{round_index}_SELECTIVE_MERGED.mid")
            ...
            d_verify=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v54_R{round_index}_D_GLOBAL_VERIFY.wav")
    ...
    report={
        ...
        "authority_note":"v5.4 steers and budgets A/B/C before rendering using D-derived Section Character. Deferred candidates are escalated when Audio margin is insufficient. Local Audio Judge, Global Coherence, Conductor Lock, and final merged-vs-D verification remain downstream authorities.",
    }
    rp=out_dir/(score.stem+"_SONICRAFT_STRINGS_v54_DECISION_TRACE.json")
    rp.write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding="utf-8")
    return report,rp

def main(argv=None):
    ap=argparse.ArgumentParser(description="SONICRAFT v5.4 Conductor-Steered Candidate Generation auto-loop.")
    ...
    try:
        r,rp=run_auto_loop_v54(a.score,a.out_dir,a.policy,a.host,a.port,a.backend,a.model_dir,a.cache_dir,
    ...
    print("SONICRAFT v5.4 Conductor-Steered Auto-Loop finished")
