"""SONICRAFT v6.2 Reproducible Performance Checkpoint / Policy-Evidence Binding.
...
- v6.2 acoustic runtime binding (weights/model manifest, renderer build, backend/framework, device capability, sample rate/render config)

It never embeds audio or MIDI bytes and does not claim bit-identical audio replay.
...
from acoustic_runtime_provenance_v62 import (
    capture_acoustic_runtime_provenance_v62, verify_acoustic_runtime_provenance_v62,
    export_in_toto_slsa_envelope_v62,
)

CHECKPOINT_SCHEMA=1
CHECKPOINT_VERSION="6.2"
CODE_FILES=(
    "runtime/compile_musicxml_strings_v62.py",
    ...
    "runtime/performance_checkpoint_v62.py",
    "runtime/acoustic_runtime_provenance_v62.py",
)
ARTIFACT_KEYS=(
...
def code_fingerprint_v62(project_root=None):
...
def policy_payload_from_snapshot_v62(snapshot):
    return {
        "version":1,
...
def policy_binding_v62(snapshot):
    payload=policy_payload_from_snapshot_v62(snapshot)
...
def _artifact_bindings(comp):
    ...
    for key in ARTIFACT_KEYS:
        p=Path(comp[key])
        out[key]={"file_name":p.name,**file_fingerprint_v62(p)}
    return out
...
def _checkpoint_identity_projection_v62(payload_without_id):
    ...
    inp=q.get("input_state",{})
    ev=inp.get("evidence",{})
    if isinstance(ev,dict):ev["pin_tag"]="@CHECKPOINT_PIN@"
    # Display paths and explanatory strings are useful for forensics but are not immutable acoustic
    # identity.  The acoustic binding itself is SHA-256 over its path-free `identity` projection.
    ar=inp.get("acoustic_runtime")
    if isinstance(ar,dict): ar.pop("forensics",None)
...
def checkpoint_id_v62(payload_without_id):
    return _sha_bytes(_canonical(_checkpoint_identity_projection_v62(payload_without_id)))[:24]


def create_compile_checkpoint_v62(comp,source_score,evidence_store,round_index,out_path=None,project_root=None,
    ...
    policy=policy_binding_v62(comp["policy_snapshot"])
    intent=comp["conductor_intent"]
    steering=comp["steering_report"]
    artifact_bindings=_artifact_bindings(comp)
    acoustic_runtime=copy.deepcopy(acoustic_runtime) if acoustic_runtime is not None else capture_acoustic_runtime_provenance_v62(project_root=project_root)
    body={
        ...
        "replay_contract":{
            ...
            "acoustic_runtime_bound":True,
            "acoustic_environment_explainable":True,
        },
    }
    # Checkpoint identity normalizes its own pin tag and path-dependent JSON raw hashes.
    cid=checkpoint_id_v62(body)
...
def _policy_snapshot_from_file(path):
    ...
    return policy_binding_v62(mem.snapshot())


def finalize_checkpoint_v62(checkpoint_path,evidence_store,policy_path,
    ...
    for name,path in (artifacts or {}).items():
        pp=Path(path)
        if pp.is_file():artifact_rows[str(name)]={"file_name":pp.name,**file_fingerprint_v62(pp)}
...
def load_checkpoint_v62(path):
    ...
    # checkpoint_id_v62 applies the path-independent immutable identity projection.
    expected=checkpoint_id_v62(clone)
...
def verify_checkpoint_v62(checkpoint_path,source_score,evidence_store,project_root=None,
                          current_policy_path=None,artifact_dir=None,acoustic_runtime_kwargs=None):
    cp=load_checkpoint_v62(checkpoint_path)
    source=Path(source_score).resolve()
    source_sha=_sha_bytes(source.read_bytes()) if source.exists() else None
    source_ok=source_sha==cp["source"]["sha256"]

    code_now=code_fingerprint_v62(project_root)
    code_ok=code_now["sha256"]==cp["compiler"]["sha256"]

    expected_runtime=cp["input_state"].get("acoustic_runtime")
    runtime_check=None
    if expected_runtime is not None:
        kwargs=dict(acoustic_runtime_kwargs or {})
        if "project_root" not in kwargs: kwargs["project_root"]=project_root
        runtime_check=verify_acoustic_runtime_provenance_v62(expected_runtime,**kwargs)
    runtime_ok=bool(runtime_check and runtime_check.get("ok")) if expected_runtime is not None else False
    ...
    return {
        ...
        "acoustic_runtime_ok":runtime_ok,"acoustic_runtime":runtime_check,
        "compile_replay_ready":bool(source_ok and code_ok and evidence_ok),
        "replay_ready":bool(source_ok and code_ok and evidence_ok),
        "acoustic_replay_context_ready":bool(source_ok and code_ok and evidence_ok and runtime_ok),
    }
...
def replay_verify_checkpoint_v62(checkpoint_path,source_score,evidence_store,project_root=None,acoustic_runtime_kwargs=None):
    """Non-destructive compile replay. Evidence is verified read-only; no global state is restored."""
    cp=load_checkpoint_v62(checkpoint_path)
    pre=verify_checkpoint_v62(checkpoint_path,source_score,evidence_store,project_root,
                              current_policy_path=None,artifact_dir=Path(checkpoint_path).parent,
                              acoustic_runtime_kwargs=acoustic_runtime_kwargs)
    ...
    with tempfile.TemporaryDirectory(prefix="sonicraft_v62_replay_") as td:
        td=Path(td)
        ...
        return {
            ...
            "acoustic_runtime_match":bool(pre.get("acoustic_runtime_ok")),
            "acoustic_runtime_differences":((pre.get("acoustic_runtime") or {}).get("differences") or []),
        }


def restore_checkpoint_environment_v62(checkpoint_path,evidence_store,evidence_paths,policy_path,
                                       backup_policy=True):
    cp=load_checkpoint_v62(checkpoint_path)
    ...
    if backup_policy and policy_path.exists():
        backup=policy_path.with_name(policy_path.name+".pre_v62_restore.bak")
...
def release_checkpoint_pin_v62(checkpoint_path,evidence_store):
    cp=load_checkpoint_v62(checkpoint_path)
...
def main(argv=None):
    ap=argparse.ArgumentParser(description="SONICRAFT v6.2 Reproducible Performance Checkpoint")
    ap.add_argument("command",choices=["verify","replay","restore","release","provenance"])
    ...
    ap.add_argument("--chunk-seconds",type=float,default=40.0)
    ap.add_argument("--overlap-seconds",type=float,default=0.75)
    ap.add_argument("--local-context",type=float,default=0.85)
    ap.add_argument("--max-local-context-seconds",type=float,default=28.0)
    a=ap.parse_args(argv)
    runtime_kwargs={"backend":a.backend,"model_dir":a.model_dir,"mock":a.mock,"sample_rate":a.sample_rate,
                    "chunk_seconds":a.chunk_seconds,"overlap_seconds":a.overlap_seconds,
                    "local_context":a.local_context,"max_local_context_seconds":a.max_local_context_seconds}
    if a.command=="provenance":
        cp=load_checkpoint_v62(a.checkpoint)
        out=export_in_toto_slsa_envelope_v62(cp["input_state"]["acoustic_runtime"],subject_name=cp["checkpoint_id"])
    ...
    if a.command=="verify":
        ...
        out=verify_checkpoint_v62(a.checkpoint,a.score,store,current_policy_path=a.policy,acoustic_runtime_kwargs=runtime_kwargs)
    elif a.command=="replay":
        if not a.score:raise SystemExit("--score required")
        out=replay_verify_checkpoint_v62(a.checkpoint,a.score,store,acoustic_runtime_kwargs=runtime_kwargs)
    elif a.command=="restore":
        if not a.policy:raise SystemExit("--policy required")
        paths=_paths_from_args(a)
        out=restore_checkpoint_environment_v62(a.checkpoint,store,paths,a.policy,True)
    elif a.command=="release":
        out=release_checkpoint_pin_v62(a.checkpoint,store)
    elif a.command=="provenance":
        raise AssertionError("provenance handled before store initialization")
