...
import tempfile
import numpy as np
import auto_loop_strings_v52 as loop
...
def fake_full(midi,out_wav,host="127.0.0.1",port=0,sample_rate=8000,chunk_seconds=40,overlap_seconds=.75,request_seed=1,**kwargs):
    n=24000;t=np.arange(n,dtype=np.float32)/sample_rate;name=Path(midi).name
    if "SELECTIVE_MERGED" in name: marker=.60
    elif "_REPAIR_B_" in name: marker=.90
    elif "_REPAIR_A_" in name: marker=.70
    elif "_REPAIR_C_" in name: marker=.66
    else: marker=.80
    x=.06*np.sin(2*np.pi*220*t);audio=np.repeat(x[:,None],2,axis=1);audio[0,0]=marker
...
with tempfile.TemporaryDirectory() as td:
    td=Path(td);score=td/"pairfail.musicxml";score.write_text(XML)
    ...
    try:
        report,rp=loop.run_auto_loop_v52(score,td/"out",td/"policy.json",port=49592,mock=True,max_round=1,
    ...
    assert report["rounds"][0]["mode"]=="selective",report["rounds"][0]
    assert report["rounds"][0]["global_coherence"]["passed"]
    assert report["final"]["mode"]=="full_fallback_pair_verify",report["final"]
    assert report["final"]["status"]=="global_pair_fallback"
    assert report["final"]["winner"]=="B",report["final"]
    assert report["final"]["pair_overall_delta"]<-.025
    assert Path(report["final"]["midi"]).exists() and Path(report["final"]["wav"]).exists()
    # Local learning may have advanced once; fallback must not advance again from the stale R1 queue.
    from string_repair_policy_v49 import RepairPolicyMemoryV49
    snap=RepairPolicyMemoryV49(td/"policy.json").snapshot()
    assert snap.generation==1,snap
    print("SONICRAFT v5.2 global pair failure -> full A/B/C/D fallback smoke OK",
          report["final"]["winner"],report["final"]["pair_overall_delta"],"policy_gen",snap.generation)
