"""SONICRAFT v5.1 Selective Phrase Search / Local Repair Auto-Loop.
...
from compile_musicxml_strings_v51 import compile_file
...
def _copy_final(src_score,out_dir,midi,wav,label):
    ...
    mo=out_dir/f"{stem}_SONICRAFT_STRINGS_v51_{label}.mid"
    wo=out_dir/f"{stem}_SONICRAFT_STRINGS_v51_{label}.wav"
...
def run_auto_loop_v51(score,out_dir=None,policy_path=None,host="127.0.0.1",port=49337,
    ...
    out_dir=Path(out_dir) if out_dir else score.with_name(score.stem+"_SONICRAFT_v51_SELECTIVE")
    ...
    try:
        ...
        for round_index in range(1,max_round+1):
            out=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v51_R{round_index}.mid")
            ...
            merged=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v51_R{round_index}_SELECTIVE_MERGED.mid")
            ...
            # Converged/mixed/round-cap: render only the merged result once at full length.
            ...
            final={
                ...
                "mode":"selective","round":round_index,
                "window_winners":[{"window_id":d["window_id"],"winner":d["winner"],"margin":d["margin"]} for d in decisions],
                "midi":str(fm),"wav":str(fw),"full_final_render":{k:(str(v) if isinstance(v,Path) else v) for k,v in finfo.items()},
                "local_render_equivalent_full":round(equiv,6),
                "estimated_total_vs_four_full_fraction":round((equiv+1.0)/4.0,6),
            }
            break
    ...
    report={
        ...
        "authority_note":"Local Audio Judge decides only localized repair windows. Diffuse or ambiguous cases fall back to full v5.0-style A/B/C/D judging.",
    }
    rp=out_dir/(score.stem+"_SONICRAFT_STRINGS_v51_DECISION_TRACE.json")
    rp.write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding="utf-8")
    return report,rp

def main(argv=None):
    ap=argparse.ArgumentParser(description="SONICRAFT v5.1 selective phrase/local-repair Shadow auto-loop.")
    ...
    try:
        r,rp=run_auto_loop_v51(a.score,a.out_dir,a.policy,a.host,a.port,a.backend,a.model_dir,a.cache_dir,
    ...
    print("SONICRAFT v5.1 Selective Phrase Auto-Loop finished")
