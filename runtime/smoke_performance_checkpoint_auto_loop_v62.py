...
import numpy as np
import auto_loop_strings_v62 as loop
from compile_musicxml_strings_v62 import compile_file as real_compile
...
from evidence_store_v60 import UnifiedEvidenceStoreV60
from performance_checkpoint_v62 import load_checkpoint_v62,replay_verify_checkpoint_v62,verify_checkpoint_v62
...
with tempfile.TemporaryDirectory() as td:
    ...
    utility=td/"utility.json";audit=td/"audit.json";transfer=td/"transfer.json";archp=td/"archetype_memory.json";mixp=td/"mixture_memory.json";storep=td/"evidence_store.json";cpdir=td/"checkpoints"
    ...
    def wrapped_compile(*args,**kwargs):
        ...
        r["archetype_json"].write_text(json.dumps({"schema":1,"version":"6.1","classification":cls.as_dict()},indent=2))
        r["mixture_json"].write_text(json.dumps({"schema":1,"version":"6.1","soft_mixture":mix.as_dict()},indent=2))
        return r
    ...
    try:
        report,rp=loop.run_auto_loop_v62(
            score,td/"out",td/"policy.json",port=49641,mock=True,max_round=1,
            ...
            archetype_memory_path=archp,mixture_memory_path=mixp,evidence_store_path=storep,checkpoint_dir=cpdir
        )
    finally:
    ...
    assert storep.exists()
    estore=UnifiedEvidenceStoreV60(storep)
    st=estore.status()
    assert st["head"] and st["commits"]>=2,st
    assert row["evidence_store_head"]==st["head"],(row["evidence_store_head"],st)
    assert row["evidence_transaction"]["commit_id"]==st["head"],row["evidence_transaction"]
    paths={"utility_v55":utility,"audit_v56":audit,"similarity_v57":transfer,
           "archetype_v58":archp,"mixture_v59":mixp}
    assert estore.verify_legacy(paths)["all_match"]

    # v6.2 checkpoint is auto-created from the PRE-round evidence/policy state and finalized after the round.
    cp_path=Path(report["final"]["performance_checkpoint"])
    assert cp_path.exists(),report["final"]
    cp=load_checkpoint_v62(cp_path)
    assert cp["output_state"] is not None,cp
    assert cp["result_binding_sha256"]==report["final"]["checkpoint_result_binding"],(cp,report["final"])
    assert cp["input_state"]["evidence"]["commit_id"]!=cp["output_state"]["evidence"]["commit_id"],cp
    assert cp["input_state"]["repair_policy"]["profile_hash"]!=cp["output_state"]["repair_policy"]["profile_hash"],cp
    cv=verify_checkpoint_v62(cp_path,score,estore,current_policy_path=td/"policy.json",artifact_dir=td/"out")
    assert cv["source_ok"] and cv["compiler_code_ok"] and cv["evidence_commit_ok"],cv
    # Current policy has advanced after local learning, which is expected.
    assert cv["current_policy_matches_input"] is False,cv
    # Replay itself is covered by smoke_performance_checkpoint_v62.py using the unmodified real compiler.
    # This Auto-Loop fixture intentionally overrides Archetype/Mixture sidecars to force a stable cold-start
    # mixture, so those two synthetic sidecars are not eligible for real-compiler replay comparison.

    # Simulate a crash/manual partial write after the committed transaction.
    clean_utility=json.loads(utility.read_text())
    drift=dict(clean_utility);drift["generation"]=999
    drift["contexts"]=dict(clean_utility.get("contexts",{}));drift["contexts"]["partial_crash_drift"]={"total_windows":99,"slots":{}}
    utility.write_text(json.dumps(drift,indent=2))
    before=estore.verify_legacy(paths)
    assert before["namespaces"]["utility_v55"]["status"]=="drift",before
    recovered=estore.bootstrap_or_recover(paths)
    assert recovered["mode"]=="recovered_to_head",recovered
    restored=json.loads(utility.read_text())
    assert restored["generation"]==clean_utility["generation"]
    assert "partial_crash_drift" not in restored.get("contexts",{})
    assert estore.verify_legacy(paths)["all_match"]
    assert estore.status()["quarantine"]>=1
    print("SONICRAFT v6.2 Auto-Loop checkpoint + replay + evidence recovery smoke OK",
          ...
          "mix_ev",cb["mixture_evidence"],"store_commits",st["commits"],"checkpoint",cp["checkpoint_id"],"quarantine",estore.status()["quarantine"],
