...
import tempfile
import numpy as np
import auto_loop_strings_v57 as loop
...
def fake_local(midi,start_tick,end_tick,out_wav=None,host="127.0.0.1",port=0,sample_rate=8000,
    ...
    name=Path(midi).name
    marker=.90 if "_REPAIR_B_" in name else (.72 if "_REPAIR_A_" in name else (.68 if "_REPAIR_C_" in name else .64))
...
with tempfile.TemporaryDirectory() as td:
    td=Path(td);score=td/"transfer.musicxml";score.write_text(XML)
    utility=td/"utility.json";audit=td/"audit.json";transfer=td/"transfer_edges.json"
    ...
    w0=plan.windows[0]
    # Exercise the v5.7 generalization path on a Build primary budget (ABCD), while the independent
    # Conductor/Coherence layers continue to use the real score intent downstream.
    ...
    assert set(core_dims)>={"transition","latent_playability"},core_dims
    target=context_key_v55("build",core_dims)
    k1=context_key_v55("build",["transition"]);k2=context_key_v55("build",["latent_playability"])
    um=CandidateUtilityMemoryV55(utility);am=CounterfactualAuditMemoryV56(audit);tm=SimilarityTransferMemoryV57(transfer)
    hist={"A":{"overall":.72,"safety":.90},"B":{"overall":.94,"safety":.93},"C":{"overall":.62,"safety":.88},"D":{"overall":.64,"safety":.93}}
    for _ in range(12):
        um.learn_rendered(k1,hist,"B",full_evidence=True);um.learn_rendered(k2,hist,"B",full_evidence=True)
    assert not um.context(target)
    pred=predict_candidate_utility_v57("build",core_dims,pre.get("steered_scores"),pre.get("reports"),
                                       pre["policy_snapshot"].values,um,am,tm,fake_budget["active"])
    assert pred.local_evidence==0,pred
    assert pred.transfer_evidence>=1.5,pred
    assert pred.reason=="similarity_transfer_top2_plus_D",pred
    assert len(pred.initial_slots)==3 and "B" in pred.initial_slots and "D" in pred.initial_slots,pred
    assert len(pred.pruned_slots)==1,pred

    o_budget=loop.render_slots_for_window_v54
    ...
    try:
        report,rp=loop.run_auto_loop_v57(score,td/"out",td/"policy.json",port=49593,mock=True,max_round=1,
                                         sample_rate=8000,coverage_limit=.55,local_context=.4,
                                         max_local_context_seconds=12,utility_memory_path=utility,
                                         audit_memory_path=audit,transfer_memory_path=transfer)
    finally:
        loop.render_slots_for_window_v54=o_budget
    ...
    assert cb["utility_reason"]=="similarity_transfer_top2_plus_D",cb
    assert cb["local_utility_evidence"]==0,cb
    assert cb["transferred_utility_evidence"]>=1.5,cb
    assert len(cb["transfer_donors"])>=2,cb
    assert len(cb["primary_slots"])==3 and len(cb["deferred_slots"])==1,cb
    ...
    assert row["candidate_renders_escalated"]==0 and row["candidate_renders_audited"]==0,row
    assert Path(row["context_similarity_transfer_sidecar"]).exists()
    # Target local memory learns only actually rendered slots; the pruned slot remains untouched.
    ...
    for s in cb["rendered_slots"]:assert float(ctx[s]["evidence"])>0,ctx
    assert report["final"]["mode"]=="selective_conductor_lock",report["final"]
    assert report["final"]["pair_verify"]["passed"]
    print("SONICRAFT v5.7 cold-target SIMILARITY TRANSFER saved one local render OK",
          "target",target,"donors",cb["transfer_donors"],"initial",cb["primary_slots"],
          "pruned",cb["deferred_slots"],"cost",round(report["final"]["estimated_total_vs_four_full_fraction"],3))
