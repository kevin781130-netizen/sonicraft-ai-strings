...
import tempfile
import numpy as np
import auto_loop_strings_v56 as loop
...
from candidate_utility_predictor_v55 import CandidateUtilityMemoryV55,context_key_v55,predict_candidate_utility_v55
...
with tempfile.TemporaryDirectory() as td:
    ...
    # Prime the deterministic auditor to 11 prune opportunities so this real auto-loop window is #12.
    pred=predict_candidate_utility_v55(bud["character"],w0.dimensions,pre.get("steered_scores"),pre.get("reports"),
                                      pre["policy_snapshot"].values,um,bud["active"])
    assert pred.initial_slots==["B","D"],pred
    audit=td/"audit.json";am=CounterfactualAuditMemoryV56(audit)
    for i in range(11):
        pp=am.plan(pred,bud["active"]);assert not pp.audit_due,(i,pp)
    ...
    try:
        report,rp=loop.run_auto_loop_v56(score,td/"out",td/"policy.json",port=49591,mock=True,max_round=1,
                                         sample_rate=8000,coverage_limit=.55,local_context=.4,
                                         max_local_context_seconds=12,utility_memory_path=utility,audit_memory_path=audit)
    finally:
    ...
    assert row["candidate_renders_audited"]>=2,row
    assert row["candidate_renders_escalated"]==0,row
    assert row["counterfactual_audit_events"],row
    ev=row["counterfactual_audit_events"][0]
    assert ev["false_prune"] is True,ev
    assert ev["preaudit_winner"]=="B" and ev["full_winner"]=="A",ev
    assert ev["counterfactual_gain"]>=.025,ev
    d0=row["decisions"][0]
    cb=d0["candidate_budget"]
    assert cb["utility_reason"]=="high_conf_top1_plus_D",cb
    assert cb["primary_slots"]==["B","D"],cb
    assert cb["standard_escalation"] is False,cb
    assert cb["counterfactual_audit"] is True,cb
    assert cb["escalate_reason"]=="accepted_initial_budget",cb
    assert set(cb["rendered_slots"])==set("ABCD"),cb
    assert d0["winner"]=="A",d0
    # Audited A/C are real renders, so unlike v5.5 Zero-Render they are now legal learning evidence.
    m2=CandidateUtilityMemoryV55(utility);ctx=m2.context(key)["slots"]
    assert ctx["A"]["evidence"]>10 and ctx["C"]["evidence"]>10,ctx
    acal=CounterfactualAuditMemoryV56(audit).calibration(key)
    assert acal["false_prunes"]==1 and acal["audits"]==1,acal
    ...
    print("SONICRAFT v5.6 scheduled COUNTERFACTUAL AUDIT caught hidden false prune OK",
          "B->A",ev["counterfactual_gain"],"audit_interval",acal["audit_interval"],
