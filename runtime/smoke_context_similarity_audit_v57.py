...
def fake_local(midi,start_tick,end_tick,out_wav=None,host="127.0.0.1",port=0,sample_rate=8000,
    ...
    name=Path(midi).name
    marker=.90 if "_REPAIR_B_" in name else (.96 if "_REPAIR_A_" in name else (.68 if "_REPAIR_C_" in name else .64))
...
with tempfile.TemporaryDirectory() as td:
    td=Path(td);score=td/"audit_transfer.musicxml";score.write_text(XML)
    utility=td/"utility.json";audit=td/"audit.json";transfer=td/"transfer.json"
    ...
    target=context_key_v55("build",core_dims);k1=context_key_v55("build",["transition"]);k2=context_key_v55("build",["latent_playability"])
    um=CandidateUtilityMemoryV55(utility);am=CounterfactualAuditMemoryV56(audit);tm=SimilarityTransferMemoryV57(transfer)
    hist={"A":{"overall":.68,"safety":.90},"B":{"overall":.94,"safety":.92},"C":{"overall":.58,"safety":.88},"D":{"overall":.64,"safety":.93}}
    for _ in range(12):
        um.learn_rendered(k1,hist,"B",True);um.learn_rendered(k2,hist,"B",True)
    donor_before={k:um.context(k)["slots"]["B"]["evidence"] for k in (k1,k2)}
    ...
    assert pred.reason=="similarity_transfer_top2_plus_D" and "A" in pred.pruned_slots,pred
    # Prime target audit opportunity to 11. This does not touch donor audit state.
    for i in range(11):
        pp=am.plan(pred,fake_budget["active"]);assert not pp.audit_due,(i,pp)
    ...
    try:
        report,rp=loop.run_auto_loop_v57(score,td/"out",td/"policy.json",port=49594,mock=True,max_round=1,
    ...
    assert row["context_similarity_transfer_events"],row
    tev=row["context_similarity_transfer_events"][0]
    assert tev["false_prune"] is True and tev["target"]==target,tev
    tm2=SimilarityTransferMemoryV57(transfer)
    for donor in (k1,k2):
        ec=tm2.calibration(target,donor)
        assert ec["trust"]<1.0 and ec["false_prunes"]==1,ec
        # Donor local utility history is not punished by transfer failure.
        assert CandidateUtilityMemoryV55(utility).context(donor)["slots"]["B"]["evidence"]==donor_before[donor]
    assert d0["winner"]=="A",d0
    print("SONICRAFT v5.7 counterfactual audit calibrated TRANSFER EDGES without poisoning donors OK",
          "B->A",ev["counterfactual_gain"],"edge_trust",[tm2.calibration(target,k)["trust"] for k in (k1,k2)])
