"""SONICRAFT v5.5 Candidate Utility Predictor / Zero-Render Pruning Auto-Loop.
...
def _copy_final(src_score,out_dir,midi,wav,label):
    ...
    mo=out_dir/f"{stem}_SONICRAFT_STRINGS_v55_{label}.mid"
    wo=out_dir/f"{stem}_SONICRAFT_STRINGS_v55_{label}.wav"
...
def run_auto_loop_v55(score,out_dir=None,policy_path=None,host="127.0.0.1",port=49337,
                      ...
                      utility_memory_path=None):
    score=Path(score).resolve()
    if not score.exists():raise FileNotFoundError(score)
    out_dir=Path(out_dir) if out_dir else score.with_name(score.stem+"_SONICRAFT_v55_SELECTIVE")
    ...
    try:
        ...
        for round_index in range(1,max_round+1):
            out=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v55_R{round_index}.mid")
            ...
            for wi,w in enumerate(plan.windows):
                ...
                active=list(prediction.initial_slots);deferred=list(prediction.pruned_slots)
                ...
                d={
                    ...
                    "candidate_budget":{
                        ...
                        "rendered_slots":list(rendered),"expanded":expanded,
            ...
            utility_path=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v55_R{round_index}.candidate_utility.json")
            ...
            conductor_path=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v55_R{round_index}.conductor_lock.json")
            ...
            merged=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v55_R{round_index}_SELECTIVE_MERGED.mid")
            ...
            d_verify=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v55_R{round_index}_D_GLOBAL_VERIFY.wav")
    ...
    report={
        ...
        "authority_note":"v5.5 predicts candidate utility before render using explainable priors plus actual-render-only local history. D always renders; low margin or predictor-vs-audio disagreement expands every pruned candidate. Audio Judge, Global Coherence, Conductor Lock and merged-vs-D verification remain downstream authorities.",
    }
    rp=out_dir/(score.stem+"_SONICRAFT_STRINGS_v55_DECISION_TRACE.json")
    rp.write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding="utf-8")
    return report,rp

def main(argv=None):
    ap=argparse.ArgumentParser(description="SONICRAFT v5.5 Conductor-Steered Candidate Generation auto-loop.")
    ...
    try:
        r,rp=run_auto_loop_v55(a.score,a.out_dir,a.policy,a.host,a.port,a.backend,a.model_dir,a.cache_dir,
    ...
    print("SONICRAFT v5.5 Conductor-Steered Auto-Loop finished")
