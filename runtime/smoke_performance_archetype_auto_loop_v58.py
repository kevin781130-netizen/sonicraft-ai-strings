...
with tempfile.TemporaryDirectory() as td:
    ...
    assert cb["archetype_label"]=="intimate" and cb["archetype_classification_confidence"]>.8,cb
    assert len(cb["primary_slots"])==3 and "D" in cb["primary_slots"],cb
    assert len(cb["deferred_slots"])==1,cb
    assert cb["expanded"] is False,cb
    assert set(cb["rendered_slots"])==set(cb["primary_slots"]),cb
    assert d0["winner"]=="B",d0
    assert row["candidate_renders_skipped"]>=1,row
    assert Path(row["performance_archetype_memory_sidecar"]).exists()
    # Cross-song archetype memory and exact Utility memory learn only actually rendered slots.
    ...
    archctx=PerformanceArchetypeMemoryV58(archp).context(cls.label,target)["slots"]
    assert pruned not in archctx or pruned=="A",archctx
    assert report["final"]["mode"]=="selective_conductor_lock",report["final"]
    assert report["final"]["pair_verify"]["passed"]
    print("SONICRAFT v5.8 CROSS-SONG ARCHETYPE cold-start saved one local render OK",
          "archetype",cls.label,"target",target,"initial",cb["primary_slots"],
          "pruned",cb["deferred_slots"],"arch_ev",cb["archetype_evidence"],
