...
from archetype_mixture_v59 import ArchetypeMixtureMemoryV59,mixture_from_distances_v59,predict_candidate_utility_v59
...
with tempfile.TemporaryDirectory() as td:
    ...
    pred0=predict_candidate_utility_v59(
        "build",core_dims,pre.get("steered_scores"),pre.get("reports"),
        pre["policy_snapshot"].values,um,am,tm,arm,mm,mix,fake_budget["active"]
    )
    assert pred0.reason=="soft_archetype_mixture_top2_plus_D",pred0
    assert "A" in pred0.pruned_slots,pred0
    for i in range(11):
        pp=am.plan(pred0,fake_budget["active"])
        assert not pp.audit_due,(i,pp)
    v58_before={lab:arm.calibration(lab,target).copy() for lab in ("intimate","ballad","chamber")}
    transfer_before=dict(tm.edges)
    ...
    assert row["archetype_mixture_audit_events"],row
    mev=row["archetype_mixture_audit_events"][0]
    assert mev["false_prune"] is True,mev
    mm2=ArchetypeMixtureMemoryV59(mixp)
    trusts={x["label"]:x["trust"] for x in mev["components"]}
    assert trusts["intimate"]<trusts["ballad"]<trusts["chamber"],trusts
    # v5.8 hard-archetype calibration and v5.7 transfer edges are untouched.
    arm2=PerformanceArchetypeMemoryV58(archp)
    for lab in ("intimate","ballad","chamber"):
        assert arm2.calibration(lab,target)==v58_before[lab],(lab,arm2.calibration(lab,target),v58_before[lab])
    assert SimilarityTransferMemoryV57(transfer).edges==transfer_before
    assert d0["winner"]=="A",d0
    assert report["final"]["pair_verify"]["passed"]
    print("SONICRAFT v5.9 SOFT MIXTURE hidden false-prune audit isolated OK",
          "B->A",ev["counterfactual_gain"],"component_trust",trusts,
          "v58_unchanged",True,"v57_edges",len(SimilarityTransferMemoryV57(transfer).edges))
