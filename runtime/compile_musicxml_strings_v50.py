"""SONICRAFT v5.0 Shadow-Auto-Loop-capable A/B/C/D compiler.
...
def compile_file(src:Path,out:Path|None=None,policy_path:Path|None=None,round_index:int=1):
    ...
    out=out or src.with_name(src.stem+f"_SONICRAFT_STRINGS_v50_R{round_index}.mid")
    out=Path(out)
    ...
    gesture_json.write_text(json.dumps({"schema":1,"version":"5.0",
    ...
    cb.update({
        "version":"5.0","round_index":round_index,
    ...
    policy_json.write_text(json.dumps({
        "schema":1,"version":"5.0","generation":snap.generation,"evidence":snap.evidence,
        "confidence":snap.confidence,"profile_hash":snap.profile_hash,"values":snap.values,
        "persistent_policy_path":str(mem.path),
    },ensure_ascii=False,indent=2),encoding="utf-8")

    queue={
        "schema":2,"version":"5.0","round_index":round_index,
        ...
        "purpose":"render A/B/C/D identically, then run v5.0 post-render iteration",
        "slots":{
        ...
        "next_command":"AUTO_LOOP_STRINGS_v50.bat <source score>",
    }
...
def main(argv=None):
    ap=argparse.ArgumentParser(description="SONICRAFT v5.0 Shadow-Auto-Loop-capable Strings compiler.")
    ...
    print("SONICRAFT v5.0 Shadow Auto-Loop Compile OK")
