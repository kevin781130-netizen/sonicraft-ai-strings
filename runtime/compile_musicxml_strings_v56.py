"""SONICRAFT v5.6 Counterfactual Render Auditor / False-Prune Self-Calibration compiler.
...
def compile_file(src:Path,out:Path|None=None,policy_path:Path|None=None,round_index:int=1):
    ...
    out=out or src.with_name(src.stem+f"_SONICRAFT_STRINGS_v56_R{round_index}.mid")
    out=Path(out)
    ...
    gesture_json.write_text(json.dumps({"schema":1,"version":"5.6",
    ...
    cb.update({
        "version":"5.6","round_index":round_index,
    ...
    policy_json.write_text(json.dumps({
        "schema":1,"version":"5.6","generation":snap.generation,"evidence":snap.evidence,
        "confidence":snap.confidence,"profile_hash":snap.profile_hash,"values":snap.values,
        "persistent_policy_path":str(mem.path),
    },ensure_ascii=False,indent=2),encoding="utf-8")

    queue={
        "schema":2,"version":"5.6","round_index":round_index,
        ...
        "purpose":"render A/B/C/D identically, then run v5.6 post-render iteration",
        "slots":{
        ...
        "next_command":"AUTO_LOOP_STRINGS_v56.bat <source score>",
    }
...
def main(argv=None):
    ap=argparse.ArgumentParser(description="SONICRAFT v5.6 Global-Coherence-capable Strings compiler.")
    ...
    print("SONICRAFT v5.6 Counterfactual Auditor Compile OK")
