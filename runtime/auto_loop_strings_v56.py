"""SONICRAFT v5.6 Counterfactual Render Auditor / False-Prune Self-Calibration Auto-Loop.
...
from compile_musicxml_strings_v56 import compile_file
...
from candidate_utility_predictor_v55 import CandidateUtilityMemoryV55,predict_candidate_utility_v55,should_escalate_v55,default_utility_path
...
def _copy_final(src_score,out_dir,midi,wav,label):
    ...
    mo=out_dir/f"{stem}_SONICRAFT_STRINGS_v56_{label}.mid"
    wo=out_dir/f"{stem}_SONICRAFT_STRINGS_v56_{label}.wav"
...
def run_auto_loop_v56(score,out_dir=None,policy_path=None,host="127.0.0.1",port=49337,
                      ...
                      utility_memory_path=None,audit_memory_path=None):
    score=Path(score).resolve()
    if not score.exists():raise FileNotFoundError(score)
    out_dir=Path(out_dir) if out_dir else score.with_name(score.stem+"_SONICRAFT_v56_SELECTIVE")
    ...
    try:
        ...
        for round_index in range(1,max_round+1):
            out=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v56_R{round_index}.mid")
            ...
            for wi,w in enumerate(plan.windows):
                ...
                prediction=predict_candidate_utility_v55(
                    v54_budget["character"],w.dimensions,comp.get("steered_scores"),comp.get("reports"),
                    comp["policy_snapshot"].values,utility_mem,v54_budget["active"]
                )
            ...
            utility_path=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v56_R{round_index}.candidate_utility.json")
            utility_path.write_text(json.dumps({
                "schema":1,"version":"5.5","memory_path":str(utility_mem.path),
            ...
            audit_path=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v56_R{round_index}.counterfactual_audit.json")
            audit_path.write_text(json.dumps({
                "schema":1,"version":"5.6","memory_path":str(audit_mem.path),
            ...
            conductor_path=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v56_R{round_index}.conductor_lock.json")
            ...
            merged=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v56_R{round_index}_SELECTIVE_MERGED.mid")
            ...
            d_verify=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v56_R{round_index}_D_GLOBAL_VERIFY.wav")
    ...
    report={
        "schema":1,"version":"5.6","source_score":str(score),"out_dir":str(out_dir.resolve()),
        ...
        "authority_note":"v5.6 periodically renders otherwise-pruned candidates to measure counterfactual False-Prune Rate. Audit calibration may lower predictor confidence or disable Zero-Render per context; it never edits musical content. Audio Judge, Global Coherence, Conductor Lock and merged-vs-D verification remain downstream authorities.",
    }
    rp=out_dir/(score.stem+"_SONICRAFT_STRINGS_v56_DECISION_TRACE.json")
    rp.write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding="utf-8")
    return report,rp

def main(argv=None):
    ap=argparse.ArgumentParser(description="SONICRAFT v5.6 Counterfactual Render Auditor auto-loop.")
    ...
    try:
        r,rp=run_auto_loop_v56(a.score,a.out_dir,a.policy,a.host,a.port,a.backend,a.model_dir,a.cache_dir,
                               ...
                               a.utility_memory,a.audit_memory)
    except Exception as ex:
        print("ERROR:",ex,file=sys.stderr);return 2
    print("SONICRAFT v5.6 Counterfactual Auditor Auto-Loop finished")
