"""SONICRAFT v5.7 Context Generalization / Similarity Transfer Auto-Loop.
...
from compile_musicxml_strings_v57 import compile_file
...
from context_similarity_transfer_v57 import SimilarityTransferMemoryV57,predict_candidate_utility_v57,default_transfer_path_v57
...
def _copy_final(src_score,out_dir,midi,wav,label):
    ...
    mo=out_dir/f"{stem}_SONICRAFT_STRINGS_v57_{label}.mid"
    wo=out_dir/f"{stem}_SONICRAFT_STRINGS_v57_{label}.wav"
...
def run_auto_loop_v57(score,out_dir=None,policy_path=None,host="127.0.0.1",port=49337,
    ...
    out_dir=Path(out_dir) if out_dir else score.with_name(score.stem+"_SONICRAFT_v57_SELECTIVE")
    ...
    try:
        ...
        for round_index in range(1,max_round+1):
            out=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v57_R{round_index}.mid")
            ...
            for wi,w in enumerate(plan.windows):
                ...
                prediction=predict_candidate_utility_v57(
                    v54_budget["character"],w.dimensions,comp.get("steered_scores"),comp.get("reports"),
                    comp["policy_snapshot"].values,utility_mem,audit_mem,transfer_mem,v54_budget["active"]
                )
                ...
                if accepted_local:
                    update=utility_mem.learn_rendered(prediction.context_key,{s:score_dict(scores[s]) for s in rendered},winner,full_evidence=(len(rendered)==4))
            ...
            utility_path=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v57_R{round_index}.candidate_utility.json")
            utility_path.write_text(json.dumps({
                "schema":1,"version":"5.7","memory_path":str(utility_mem.path),
            ...
            audit_path=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v57_R{round_index}.counterfactual_audit.json")
            audit_path.write_text(json.dumps({
                "schema":1,"version":"5.7","memory_path":str(audit_mem.path),
            ...
            transfer_path=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v57_R{round_index}.context_similarity_transfer.json")
            transfer_path.write_text(json.dumps({
                "schema":1,"version":"5.7","memory_path":str(transfer_mem.path),
            ...
            conductor_path=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v57_R{round_index}.conductor_lock.json")
            ...
            merged=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v57_R{round_index}_SELECTIVE_MERGED.mid")
            ...
            d_verify=out_dir/(score.stem+f"_SONICRAFT_STRINGS_v57_R{round_index}_D_GLOBAL_VERIFY.wav")
    ...
    report={
        "schema":1,"version":"5.7","source_score":str(score),"out_dir":str(out_dir.resolve()),
        ...
        "authority_note":"v5.7 borrows discounted Utility evidence only from same-character, overlapping-problem contexts. Transfer-only evidence cannot unlock Top1+D; target-local evidence is required. Counterfactual false-prunes calibrate target<-donor edges independently. Audio Judge, v5.6 Auditor, Global Coherence, Conductor Lock and merged-vs-D verification remain downstream authorities.",
    }
    rp=out_dir/(score.stem+"_SONICRAFT_STRINGS_v57_DECISION_TRACE.json")
    rp.write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding="utf-8")
    return report,rp

def main(argv=None):
    ap=argparse.ArgumentParser(description="SONICRAFT v5.7 Context Generalization / Similarity Transfer auto-loop.")
    ...
    try:
        r,rp=run_auto_loop_v57(a.score,a.out_dir,a.policy,a.host,a.port,a.backend,a.model_dir,a.cache_dir,
                               ...
                               a.utility_memory,a.audit_memory,a.transfer_memory)
    except Exception as ex:
        print("ERROR:",ex,file=sys.stderr);return 2
    print("SONICRAFT v5.7 Context Similarity Transfer Auto-Loop finished")
